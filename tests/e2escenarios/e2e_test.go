...
var _ = Describe("E2E Test", func() {
	...
	Context("starting with empty Directory", func() {
		It("should verify developer workflow from empty Directory", func() {
			...
			// "execute particle engine dev and add changes to application"
			...
			// "exit dev mode and run particle engine deploy"
			devSession.Stop()
			devSession.WaitEnd()
			...
			// "run particle engine delete and check if the component is deleted"
			command = []string{"particle engine", "delete", "component"}
			_, err = helper.RunInteractive(command, nil, func(ctx helper.InteractiveContext) {
	...
	Context("starting with non-empty Directory add Binding", Label(helper.LabelServiceBinding), func() {
		...
		It("should verify developer workflow of using binding as env in innerloop", func() {
			...
			helper.Cmd("particle engine", "add", "binding", "--name", bindingName, "--service", "cluster-example-initdb", "--bind-as-files=false").ShouldPass()
			...
			// check particle engine describe to check for env
			stdout := helper.Cmd("particle engine", "describe", "binding").ShouldPass().Out()
			helper.MatchAllInOutput(stdout, []string{"Available binding information:", "CLUSTER_HOST", "CLUSTER_PASSWORD", "CLUSTER_USERNAME"})

			// "running particle engine list"
			stdout = helper.Cmd("particle engine", "list").ShouldPass().Out()
			...
			helper.Cmd("particle engine", "remove", "binding", "--name", bindingName).ShouldPass()

			err = devSession.WaitSync()
			Expect(err).To(BeNil())
			Eventually(func() string { return helper.Cmd("particle engine", "describe", "binding").ShouldRun().Out() }).
