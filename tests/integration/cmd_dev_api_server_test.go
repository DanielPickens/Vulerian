...
import (
	...
	"github.com/daniel-pickens/particle engine/pkg/labels"
	"github.com/daniel-pickens/particle engine/tests/helper"
)

var _ = Describe("particle engine dev command with api server tests", Label(helper.LabelSkipOnOpenShift), func() {
	var cmpName string
	...
	for _, podman := range []bool{false, true} {
		...
		When("the component is bootstrapped", helper.LabelPodmanIf(podman, func() {
			...
			for _, customPort := range []bool{false, true} {
				...
				When(fmt.Sprintf("particle engine dev is run with --api-server flag (custom api server port=%v)", customPort), func() {
		...
		When("the component is bootstrapped", helper.LabelPodmanIf(podman, func() {
			...
			When("particle engine dev is run with --api-server flag", func() {
				var (
				...
				It("should serve endpoints", func() {
					...
					By("GETting /component", func() {
						...
						helper.JsonPathContentIs(strBody, "devfileData.supportedparticle engineFeatures.dev", "true")
						helper.JsonPathContentIs(strBody, "devfileData.supportedparticle engineFeatures.deploy", "false")
						helper.JsonPathContentIs(strBody, "devfileData.supportedparticle engineFeatures.debug", "false")
						helper.JsonPathContentIs(strBody, "managedBy", "particle engine")
						if podman {
			...
			When("particle engine is executed with --no-watch and --api-server flags", helper.LabelPodmanIf(podman, func() {
