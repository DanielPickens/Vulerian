...
var _ = Describe("particle engine remove binding command tests", Label(helper.LabelServiceBinding), Label(helper.LabelSkipOnOpenShift), func() {
	...
	var _ = BeforeEach(func() {
		...
		// Note: We do not add any operators here because `particle engine remove binding` is simply about removing the ServiceBinding from devfile.
	})
	...
	for _, bindingName := range []string{"my-nodejs-app-cluster-sample-k8s", "my-nodejs-app-cluster-sample-ocp"} {
		...
		When(fmt.Sprintf("the component with binding is bootstrapped (bindingName=%s)", bindingName), func() {
			BeforeEach(func() {
				...
				helper.Cmd("particle engine", "init", "--name", "mynode", "--devfile-path", helper.GetExamplePath("source", "devfiles", "nodejs", "devfile-with-service-binding-files.yaml")).ShouldPass()
			})
			...
			It("should fail to remove binding that does not exist", func() {
				helper.Cmd("particle engine", "remove", "binding", "--name", "my-binding").ShouldFail()
			})

			When("particle engine dev is running", func() {
				...
				When("binding is removed", func() {
					...
					It("should have led particle engine dev to delete ServiceBinding from the cluster", func() {
