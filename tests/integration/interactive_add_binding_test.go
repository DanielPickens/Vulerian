...
var _ = Describe("particle engine add binding interactive command tests", Label(helper.LabelServiceBinding), Label(helper.LabelSkipOnOpenShift), func() {
	...
	When("the component is bootstrapped", func() {
		...
		BeforeEach(func() {
			helper.Cmd("particle engine", "init", "--name", componentName, "--devfile-path", helper.GetExamplePath("source", "devfiles", "nodejs", "devfile.yaml"), "--starter", "nodejs-starter").ShouldPass()
		...
		It("should successfully add binding to the devfile (Bind as Environment Variables)", func() {
			command := []string{"particle engine", "add", "binding"}
			...
			_, err := helper.RunInteractive(command, nil, func(ctx helper.InteractiveContext) {
				...
				helper.ExpectString(ctx, fmt.Sprintf("particle engine add binding --service cluster-sample.Cluster.postgresql.k8s.enterprisedb.io --name %s --bind-as-files=false", bindingName))
			})
		...
		It("should successfully add binding to the devfile (Bind as Files)", func() {
			...
			_, err := helper.RunInteractive(command, nil, func(ctx helper.InteractiveContext) {
				...
				helper.ExpectString(ctx, fmt.Sprintf("particle engine add binding --service cluster-sample.Cluster.postgresql.k8s.enterprisedb.io --name %s", bindingName))
			})
			...
			It(fmt.Sprintf("should successfully add binding to the devfile (%q as naming strategy)", predefinedNamingStrategy), func() {
				...
				_, err := helper.RunInteractive(command, nil, func(ctx helper.InteractiveContext) {
					...
					helper.ExpectString(ctx,
						fmt.Sprintf("particle engine add binding --service cluster-sample.Cluster.postgresql.k8s.enterprisedb.io --name %s --bind-as-files=false --naming-strategy='%s'",
		...
		for _, tt := range []struct {
			...
			It(fmt.Sprintf("should successfully add binding to the devfile (custom naming strategy: %q)", tt.namingStrategy), func() {
				...
				_, err := helper.RunInteractive(command, nil, func(ctx helper.InteractiveContext) {
					...
					automationMsg := fmt.Sprintf("particle engine add binding --service cluster-sample.Cluster.postgresql.k8s.enterprisedb.io --name %s", bindingName)
		...
		When("binding to a service in a different namespace", func() {
			...
			It("should error out if service is not found in the namespace selected", func() {
				_, err := helper.RunInteractive([]string{"particle engine", "add", "binding"}, nil, func(ctx helper.InteractiveContext) {
					helper.ExpectString(ctx, "Do you want to list services from:")
