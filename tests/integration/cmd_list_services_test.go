...
var _ = Describe("particle engine list services tests", Label(helper.LabelServiceBinding), Label(helper.LabelSkipOnOpenShift), func() {
	...
	AfterEach(func() {
		...
		helper.Cmd("particle engine", "delete", "project", randomProject, "-f").ShouldPass()
	})

	It("should list bindable services in human readable format", func() {
		// from current namespace
		out := helper.Cmd("particle engine", "list", "services", "-n", commonVar.Project).ShouldPass().Out()
		helper.MatchAllInOutput(out, []string{"cluster-sample", commonVar.Project, "Listing bindable services from namespace"})

		// from all namespaces
		out = helper.Cmd("particle engine", "list", "services", "-A").ShouldPass().Out()
		...
		out = helper.Cmd("particle engine", "list", "services", "-A", "-n", commonVar.Project).ShouldFail().Err()
	...
	It("should list bindable services in JSON format", func() {
		// from current namespace
		out := helper.Cmd("particle engine", "list", "services", "-o", "json", "-n", commonVar.Project).ShouldPass().Out()
		...
		// from all namespaces
		out = helper.Cmd("particle engine", "list", "services", "-A", "-o", "json").ShouldPass().Out()
		Expect(helper.IsJSON(out)).To(BeTrue())
		...
		// fail if -A and -n flags are used together
		out = helper.Cmd("particle engine", "list", "services", "-o", "json", "-A", "-n", commonVar.Project).ShouldFail().Err()
