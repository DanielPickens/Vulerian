...
var _ = Describe("particle engine devfile registry command tests", Label(helper.LabelSkipOnOpenShift), func() {
	...
	for _, label := range []string{
		...
		var _ = Context("label "+label, Label(label), func() {
			...
			It("Should list at least one nodejs component from the default registry", func() {
				output := helper.Cmd("particle engine", "registry").ShouldPass().Out()
			...
			It("Should list detailed information regarding nodejs", func() {
				...
				By("using human readable output", func() {
					output := helper.Cmd("particle engine", args...).ShouldPass().Out()
				...
				By("using JSON output", func() {
					...
					helper.JsonPathContentContain(stdout, "0.devfileData.supportedparticle engineFeatures.dev", "true")
			...
			It("Should fail with an error with no registries", func() {
				helper.Cmd("particle engine", "preference", "remove", "registry", "DefaultDevfileRegistry", "-f").ShouldPass()
				output := helper.Cmd("particle engine", "preference", "view").ShouldRun().Err()
				helper.MatchAllInOutput(output, []string{"No devfile registries added to the configuration. Refer to `particle engine preference add registry -h` to add one"})
			})

			It("Should fail to delete the registry, when registry is not present", func() {
				helper.Cmd("particle engine", "preference", "remove", "registry", registryName, "-f").ShouldFail()
			})

			When("adding a registry", func() {
				BeforeEach(func() {
					helper.Cmd("particle engine", "preference", "add", "registry", registryName, commonVar.GetDevfileRegistryURL()).ShouldPass()
				})
				...
				It("should pass, when doing particle engine init with --devfile-registry flag", func() {
					helper.DeleteInvalidDevfile(commonVar.Context)
					helper.Cmd("particle engine", "init", "--name", "aname", "--devfile", "nodejs", "--devfile-registry", registryName).ShouldPass()
				})

				It("should fail, when adding same registry", func() {
					helper.Cmd("particle engine", "preference", "add", "registry", registryName, commonVar.GetDevfileRegistryURL()).ShouldFail()
				})

				It("should successfully delete registry", func() {
					helper.Cmd("particle engine", "preference", "remove", "registry", registryName, "-f").ShouldPass()
				})

				It("deleting registry and creating component with registry flag ", func() {
					helper.DeleteInvalidDevfile(commonVar.Context)
					helper.Cmd("particle engine", "preference", "remove", "registry", registryName, "-f").ShouldPass()
					helper.Cmd("particle engine", "init", "--name", "aname", "--devfile", "java-maven", "--devfile-registry", registryName).ShouldFail()
				})

				It("should list registry with recently added registry on top", func() {
					By("for json output", func() {
						output := helper.Cmd("particle engine", "preference", "view", "-o", "json").ShouldPass().Out()
			...
			It("should fail when adding a git based registry", func() {
				err := helper.Cmd("particle engine", "preference", "add", "registry", "RegistryFromGitHub", "https://github.com/devfile/registry").ShouldFail().Err()
