...
var _ = Describe("particle engine delete command tests", Label(helper.LabelSkipOnOpenShift), func() {
	...
	for _, ctx := range []struct {
		...
		devfileName string
		setupFunc   func()
		// Tparticle engine(pvala): Find a better solution to renaming a resource when the data is in a different location
			...
			for _, podman := range []bool{true, false} {
				...
				When("the component is deployed in DEV mode and dev mode stopped", helper.LabelPodmanIf(podman, func() {
					...
					AfterEach(func() {
						...
						helper.Cmd("particle engine", args...).ShouldPass()
					})
					...
					for _, runningIn := range []string{"", "dev", "deploy"} {
						...
						When(fmt.Sprintf("the component is deleted using its name (and namespace) from another directory (running-in=%q)", runningIn), func() {
							...
							BeforeEach(func() {
								...
								out = helper.Cmd("particle engine", append(args, "-f")...).ShouldPass().Out()
							})
							...
							if !podman {
								When("particle engine delete command is run again with nothing deployed on the cluster", func() {
									var stdOut string
									BeforeEach(func() {
									...
									It("should output that there are no resources to be deleted", func() {
										...
										Eventually(func() string {
											stdOut = helper.Cmd("particle engine", append(args, "-f")...).ShouldPass().Out()
											return stdOut
						...
						Context("the component is deleted while having access to the devfile.yaml", func() {
							When("the component is deleted with --files", func() {
								...
								It("should have deleted the component", func() {
									...
									deletableFileNames := []string{util.Dotparticle engineDirectory, "devfile.yaml"}
					...
					for _, withFiles := range []bool{true, false} {
						...
						When(fmt.Sprintf("a resource is changed in the devfile and the component is deleted while having access to the devfile.yaml with --files=%v --running-in=%v",
							...
							It("should delete the component", func() {
								if runningIn == "dev" {
									...
									By("showing warning about undeleted service belonging to the component", func() {
										Expect(stdout).To(SatisfyAll(
											...
											ContainSubstring("particle engine delete component --name %s --namespace %s", cmpName, commonVar.Project),
										))
									})
								}
								...
								files := helper.ListFilesInDir(commonVar.Context)
								if withFiles {
									By("ensuring that devfile.yaml has been removed because it was created with particle engine init", func() {
	...
	for _, withFiles := range []bool{true, false} {
		...
		When("deleting a component containing preStop event that is deployed with DEV and --files="+strconv.FormatBool(withFiles), func() {
			...
			BeforeEach(func() {
				...
				cmpName = "nodejs"
				helper.CopyExample(filepath.Join("source", "devfiles", "nodejs", "project"), commonVar.Context)
				helper.Cmd("particle engine", "init", "--name", cmpName, "--devfile-path", helper.GetExamplePath("source", "devfiles", "nodejs", "devfile-with-valid-events.yaml")).ShouldPass()
				session := helper.CmdRunner("particle engine", "dev", "--random-ports")
				defer session.Kill()
	...
	When("running particle engine deploy for an exec command bound to fail", func() {
		BeforeEach(func() {
			...
			helper.Cmd("particle engine", "deploy").WithTerminate(5, nil).ShouldRun()
		})
