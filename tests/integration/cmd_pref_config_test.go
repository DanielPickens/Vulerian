...
const promptMessageSubString = "Help particle engine improve by allowing it to collect usage data."

var _ = Describe("particle engine preference and config command tests", Label(helper.LabelSkipOnOpenShift), func() {

	// Tparticle engine: A neater way to provide particle engine path. Currently we assume particle engine and oc in $PATH already.
		...
		var _ = Context("label "+label, Label(label), func() {
			...
			Context("check that help works", func() {
				It("should display help info", func() {
					...
					for _, helpArg := range helpArgs {
						appHelp := helper.Cmd("particle engine", helpArg).ShouldPass().Out()
						Expect(appHelp).To(ContainSubstring(`Use "particle engine [command] --help" for more information about a command.`))
					}
				})
			})
			...
			Context("when running help for preference command", func() {
				It("should display the help", func() {
					appHelp := helper.Cmd("particle engine", "preference", "-h").ShouldPass().Out()
					Expect(appHelp).To(ContainSubstring("Modifies particle engine specific configuration settings"))
				})
			})
			...
			Context("When viewing global config", func() {
				...
				// we set the GLOBALparticle engineCONFIG variable to a value in new context
				var _ = JustBeforeEach(func() {
					newContext = helper.CreateNewContext()
					os.Setenv("GLOBALparticle engineCONFIG", filepath.Join(newContext, "preference.yaml"))
				})
				var _ = JustAfterEach(func() {
					helper.DeleteDir(newContext)
				})
				It("should get the default global config keys", func() {
					configOutput := helper.Cmd("particle engine", "preference", "view").ShouldPass().Out()
				...
				It("should get the default global config keys in JSON output", func() {
					res := helper.Cmd("particle engine", "preference", "view", "-o", "json").ShouldPass()
			...
			Context("When configuring global config values", func() {
				...
				It("should successfully updated", func() {
					for _, pref := range preferences {
						...
						helper.Cmd("particle engine", firstCmdArgs...).ShouldPass()
						value := helper.GetPreferenceValue(pref.name)
						Expect(value).To(ContainSubstring(pref.value))

						helper.Cmd("particle engine", "preference", "set", "-f", pref.name, pref.updateValue).ShouldPass()
						value = helper.GetPreferenceValue(pref.name)
						Expect(value).To(ContainSubstring(pref.updateValue))

						helper.Cmd("particle engine", "preference", "unset", "-f", pref.name).ShouldPass()
						value = helper.GetPreferenceValue(pref.name)
						Expect(value).To(BeEmpty())
					}
					globalConfPath := os.Getenv("HOME")
					os.RemoveAll(filepath.Join(globalConfPath, ".particle engine"))
				})
			})
			When("when preference.yaml contains an int value for Timeout", func() {
				BeforeEach(func() {
					preference := `
kind: Preference
apiversion: particle engine.dev/v1alpha1
particle engineSettings:
...
`
					...
					Expect(err).To(BeNil())
					os.Setenv("GLOBALparticle engineCONFIG", preferencePath)
				})
				It("should show warning about incompatible Timeout value when viewing preferences", func() {
					errOut := helper.Cmd("particle engine", "preference", "view").ShouldPass().Err()
			...
			It("should fail to set an incompatible format for a preference that accepts duration", func() {
				errOut := helper.Cmd("particle engine", "preference", "set", "RegistryCacheTime", "1d").ShouldFail().Err()
			...
			Context("When no ConsentTelemetry preference value is set", func() {
				var _ = JustBeforeEach(func() {
					...
					helper.Cmd("particle engine", "preference", "unset", "ConsentTelemetry", "-f").ShouldPass()
				})

				It("should not prompt when user calls for help", func() {
					output := helper.Cmd("particle engine", "init", "--help").ShouldPass().Out()
					Expect(output).ToNot(ContainSubstring(promptMessageSubString))
				})

				It("should not prompt when preference command is run", func() {
					output := helper.Cmd("particle engine", "preference", "view").ShouldPass().Out()
					Expect(output).ToNot(ContainSubstring(promptMessageSubString))

					output = helper.Cmd("particle engine", "preference", "set", "timeout", "5s", "-f").ShouldPass().Out()
					Expect(output).ToNot(ContainSubstring(promptMessageSubString))

					output = helper.Cmd("particle engine", "preference", "unset", "timeout", "-f").ShouldPass().Out()
					Expect(output).ToNot(ContainSubstring(promptMessageSubString))
				})
			})
			...
			Context("When ConsentTelemetry preference value is set", func() {
				...
				It("should not prompt the user", func() {
					...
					helper.Cmd("particle engine", "preference", "set", "ConsentTelemetry", "false", "-f").ShouldPass()
					output := helper.Cmd("particle engine", "init", "--name", "aname", "--devfile-path", helper.GetExamplePath("source", "devfiles", "nodejs", "devfile-registry.yaml")).ShouldPass().Out()
					Expect(output).ToNot(ContainSubstring(promptMessageSubString))
				})
			})
			...
			When("telemetry is enabled", func() {
				...
				for _, tt := range []struct {
					...
					When("unsetting value for preference "+tt.prefParam, func() {
						BeforeEach(func() {
							helper.Cmd("particle engine", "preference", "unset", tt.prefParam, "--force").ShouldPass()
						})

						It("should track parameter that is unset without any value", func() {
							helper.Cmd("particle engine", "preference", "unset", tt.prefParam, "--force").ShouldPass()
							td := helper.GetTelemetryDebugData()
							Expect(td.Event).To(ContainSubstring("particle engine preference unset"))
					...
					When("setting value for preference "+tt.prefParam, func() {
						BeforeEach(func() {
							...
							helper.Cmd("particle engine", "preference", "set", tt.prefParam, tt.value, "--force").ShouldPass()
						})

						It(fmt.Sprintf("should track parameter that is set along with its %s value", form), func() {
							td := helper.GetTelemetryDebugData()
							Expect(td.Event).To(ContainSubstring("particle engine preference set"))
							Expect(td.Properties.Success).To(BeTrue())
							Expect(td.Properties.Error).To(BeEmpty())
							Expect(td.Properties.ErrorType).To(BeEmpty())
							Expect(td.Properties.CmdProperties[segmentContext.Flags]).To(Equal("force"))
						...
						if !tt.clearText {
							...
							It("should anonymize values set such that different strings will not have same hash", func() {
								...
								helper.Cmd("particle engine", "preference", "set", tt.prefParam, tt.differentValueIfAnonymized, "--force").ShouldPass()
								td = helper.GetTelemetryDebugData()
								Expect(td.Properties.CmdProperties[segmentContext.PreferenceValue]).ShouldNot(BeEmpty())
								pref2Val, ok := td.Properties.CmdProperties[segmentContext.PreferenceValue].(string)
								Expect(ok).To(BeTrue(), fmt.Sprintf("value recorded in telemetry for preference %q is expected to be a string", tt.prefParam))
				...
				When("telemetry is enabled in preferences", func() {
					...
					When("setting ConsentTelemetry to false", func() {
						BeforeEach(func() {
							helper.Cmd("particle engine", "preference", "set", "ConsentTelemetry", "false", "--force").ShouldPass()
						})

						// https://github\.com/danielpickens/particle engine/issues/6790
						It("should record the particle engine-preference-set command in telemetry", func() {
							td := helper.GetTelemetryDebugData()
							Expect(td.Event).To(ContainSubstring("particle engine preference set"))
							Expect(td.Properties.Success).To(BeTrue())
							Expect(td.Properties.Error).To(BeEmpty())
				...
				When("Telemetry is disabled in preferences", func() {
					...
					When("setting ConsentTelemetry to true", func() {
						BeforeEach(func() {
							helper.Cmd("particle engine", "preference", "set", "ConsentTelemetry", "true", "--force").ShouldPass()
						})
	...
	When("DevfileRegistriesList CRD is installed on cluster", func() {
		BeforeEach(func() {
			...
			//Tparticle engine: install clusterRegestryList from scripts
...
`, registryURL))
			...
			It("registry list should return registry listed in CR", func() {
				stdout, stderr := helper.Cmd("particle engine", "preference", "view", "-o", "json").ShouldPass().OutAndErr()
				Expect(stderr).To(BeEmpty())
				Expect(helper.IsJSON(stdout)).To(BeTrue())
				...
				By("ensuring the last registries returned are those coming from the particle engine preferences", func() {
			...
			It("should fail to delete the in-cluster registry", func() {
				...
				stderr := helper.Cmd("particle engine", "preference", "remove", "registry", regName).ShouldFail().Err()
				Expect(stderr).Should(ContainSubstring("failed to remove registry: registry %q doesn't exist or it is not managed by particle engine", regName))
			})
		})
	})
})
