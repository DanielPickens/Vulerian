...
import (
	...
	"github.com/daniel-pickens/particle engine/tests/helper"
)

var _ = Describe("particle engine add binding command tests", Label(helper.LabelServiceBinding), Label(helper.LabelSkipOnOpenShift), func() {
	// Tparticle engine: Add integration tests for the following cases when SBO is not installed
	// * servicebinding with envvars does not send `particle engine dev` in an infinite loop
	// * `particle engine dev` deletes service binding secrets for SB that is not present locally
	...
	It("should fail creating a binding without workload parameter", func() {
		stderr := helper.Cmd("particle engine", "add", "binding", "--name", "aname", "--service", "cluster-sample").ShouldFail().Err()
		Expect(stderr).To(ContainSubstring("missing --workload parameter"))
	})

	It("should create a binding using the workload parameter", func() {
		stdout := helper.Cmd("particle engine", "add", "binding",
			"--name", "aname",
...
`))
	...
	When("binding to a service in a different namespace", func() {
		...
		It("should error out if service is not found in the namespace selected", func() {
			stderr := helper.Cmd("particle engine", "add", "binding",
				"--name", "aname",
		...
		It("should create a binding using the workload parameter", func() {
			helper.Cmd("particle engine", "add", "binding",
				"--name", "aname",
	...
	When("the component is bootstrapped", func() {
		BeforeEach(func() {
			helper.Cmd("particle engine", "init", "--name", "mynode", "--devfile-path", helper.GetExamplePath("source", "devfiles", "nodejs", "devfile.yaml"), "--starter", "nodejs-starter").ShouldPass()
		})

		It("should fail using the --workload parameter", func() {
			stderr := helper.Cmd("particle engine", "add", "binding", "--name", "aname", "--service", "cluster-sample", "--workload", "app/Deployment.apps").ShouldFail().Err()
		...
		for _, ctx := range []struct {
		...
		}{
			{
				...
				beforeEachFunc: func(bindingName string) string {
					helper.Cmd("particle engine", "add", "binding", "--name", bindingName, "--service", "cluster-sample").ShouldPass()
    ...
    version: v1`, bindingName)))
			...
			},
			{
				...
				beforeEachFunc: func(bindingName string) string {
					...
					helper.Cmd("particle engine", "add", "binding", "--name", bindingName,
  ...
  - group: postgresql.k8s.enterprisedb.io
    ...
    version: v1`, bindingName, ns)))
			...
			When("adding a binding ("+ctx.name+")", func() {
				...
				When("particle engine dev is run", func() {
					BeforeEach(func() {
				...
				When("particle engine dev is run", func() {
					...
					When("particle engine dev command is stopped", func() {
						BeforeEach(func() {
							devSession.Stop()
							devSession.WaitEnd()
						})
		...
		When("no bindable instance is present on the cluster", func() {
			...
			It("should fail to add binding with no bindable instance found error message", func() {
				errOut := helper.Cmd("particle engine", "add", "binding", "--name", "my-binding", "--service", "cluster-sample").ShouldFail().Err()
