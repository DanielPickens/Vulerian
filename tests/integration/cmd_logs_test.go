...
var _ = Describe("particle engine logs command tests", Label(helper.LabelSkipOnOpenShift), func() {
	...
	When("in a devfile directory", func() {
		BeforeEach(func() {
			...
			helper.Cmd("particle engine", "init", "--name", componentName, "--devfile-path", helper.GetExamplePath("source", "devfiles", "nodejs", "devfile.yaml")).ShouldPass()
		})
		When("not connected to any cluster or podman", Label(helper.LabelNoCluster), func() {
			It("particle engine logs should fail with an error message", func() {
				cmd := helper.Cmd("particle engine", "logs")
				stderr := cmd.ShouldFail().Err()
				Expect(stderr).To(ContainSubstring("unable to access the cluster"))
			})

			It("particle engine logs --platform podman should fail with an error message", func() {
	...
	for _, podman := range []bool{false, true} {
		...
		When("particle engine logs is executed for a component that's not running in any modes", helper.LabelPodmanIf(podman, func() {
			BeforeEach(func() {
				helper.CopyExample(filepath.Join("source", "nodejs"), commonVar.Context)
				helper.Cmd("particle engine", "init", "--name", componentName, "--devfile-path", helper.GetExamplePath("source", "devfiles", "nodejs", "devfile-deploy-functional-pods.yaml")).ShouldPass()
				Expect(helper.VerifyFileExists(".particle engine/env/env.yaml")).To(BeFalse())
			})
	...
	When("component is created and particle engine logs is executed", func() {
		BeforeEach(func() {
			helper.CopyExample(filepath.Join("source", "nodejs"), commonVar.Context)
			helper.Cmd("particle engine", "init", "--name", componentName, "--devfile-path", helper.GetExamplePath("source", "devfiles", "nodejs", "devfile-deploy-functional-pods.yaml")).ShouldPass()
			Expect(helper.VerifyFileExists(".particle engine/env/env.yaml")).To(BeFalse())
		...
		for _, podman := range []bool{false, true} {
			...
			When("running in Dev mode", helper.LabelPodmanIf(podman, func() {
				...
				It("should successfully show logs of the running component", func() {
					...
					if podman {
						// Tparticle engine(feloy): Kubernetes components are not deployed on dev for now
					...
					// `particle engine logs --dev`
					cmd = getLogCommand(podman, "--dev")
					out = cmd.ShouldPass().Out()
					helper.MatchAllInOutput(out, expectedInDev)

					// `particle engine logs --deploy`
		...
		When("running in Deploy mode", func() {
			...
			It("should successfully show logs of the running component", func() {
				// `particle engine logs`
				out := helper.Cmd("particle engine", "logs").ShouldPass().Out()
				helper.MatchAllInOutput(out, []string{"main:", "main[1]:", "main[2]:"})

				// `particle engine logs --dev`
				out = helper.Cmd("particle engine", "logs", "--dev").ShouldPass().Out()
				Expect(out).To(ContainSubstring("no containers running in the specified mode for the component"))

				// `particle engine logs --deploy`
				out = helper.Cmd("particle engine", "logs", "--deploy").ShouldPass().Out()
				helper.MatchAllInOutput(out, []string{"main:", "main[1]:", "main[2]:"})
			})
			When("--follow flag is specified", func() {
				var logsSession helper.LogsSession
		...
		When("running in both Dev and Deploy mode", func() {
			...
			It("should successfully show logs of the running component", func() {
				...
				// `particle engine logs --dev --deploy`
				out = helper.Cmd("particle engine", "logs", "--deploy", "--dev").ShouldFail().Err()
