...
var _ = Describe("particle engine dev debug command tests", func() {
	var cmpName string
	...
	for _, podman := range []bool{false, true} {
		...
		When("a component is bootstrapped", Label(helper.LabelSkipOnOpenShift), func() {
			BeforeEach(func() {
				...
				helper.Cmd("particle engine", "init", "--name", cmpName, "--devfile-path", helper.GetExamplePath("source", "devfiles", "nodejs", "devfile-with-debugrun.yaml")).ShouldPass()
				Expect(helper.VerifyFileExists(".particle engine/env/env.yaml")).To(BeFalse())
			})
			When("running particle engine dev with debug flag and custom port mapping for port forwarding", helper.LabelPodmanIf(podman, func() {
				var (
			...
			When("running particle engine dev with debug flag", helper.LabelPodmanIf(podman, func() {
	...
	for _, podman := range []bool{false, true} {
		podman := podman
		When("creating nodejs component, doing particle engine dev and run command has dev.particle engine.push.path attribute", Label(helper.LabelSkipOnOpenShift), helper.LabelPodmanIf(podman, func() {
			var devSession helper.DevSession
			var devStarted bool
			BeforeEach(func() {
	...
	for _, devfileHandlerCtx := range []struct {
		...
		for _, podman := range []bool{false, true} {
			...
			When("running build and debug commands as composite in different containers and a shared volume - "+devfileHandlerCtx.name, Label(helper.LabelSkipOnOpenShift), helper.LabelPodmanIf(podman, func() {
				...
				BeforeEach(func() {
					// Tparticle engine(rm3l): For some reason, this does not work on Podman
					if podman {
	...
	for _, podman := range []bool{false, true} {
		...
		When("a component without debug command is bootstrapped", Label(helper.LabelSkipOnOpenShift), helper.LabelPodmanIf(podman, func() {
			BeforeEach(func() {
				helper.CopyExample(filepath.Join("source", "devfiles", "nodejs", "project"), commonVar.Context)
				helper.Cmd("particle engine", "init", "--name", cmpName, "--devfile-path", helper.GetExamplePath("source", "devfiles", "nodejs", "devfile-without-debugrun.yaml")).ShouldPass()
				Expect(helper.VerifyFileExists(".particle engine/env/env.yaml")).To(BeFalse())
			})

			It("should log error about missing debug command when running particle engine dev --debug", func() {
				devSession, err := helper.StartDevMode(helper.DevSessionOpts{
					RunOnPodman: podman,
					CmdlineArgs: []string{"--debug"},
				})
		...
		When("starting with Devfile with autoBuild or deployByDefault components",
			...
			helper.LabelPodmanIf(podman, func() {
				...
				When("running particle engine dev with some components referenced in the Devfile", func() {
					...
					BeforeEach(func() {
						var err error
						//Tparticle engine (rm3l): we do not support passing a custom debug command yet. That's why we are manually updating the Devfile to change the default debug command.
