...
import (
	...
	particle enginelog "github\.com/danielpickens/particle engine/pkg/log"
	"github\.com/danielpickens/particle engine/tests/helper"
)

var _ = Describe("particle engine init interactive command tests", Label(helper.LabelSkipOnOpenShift), func() {
	...
	for _, label := range []string{
		...
		var _ = Context("label "+label, Label(label), func() {
			It("should not fail when using -v flag", func() {
				command := []string{"particle engine", "init", "-v", "4"}
			...
			Context("personalizing configuration", func() {
				It("should allow to add and delete a port", func() {
					command := []string{"particle engine", "init"}
					output, err := helper.RunInteractive(command, nil, func(ctx helper.InteractiveContext) {
			...
			It("should print automation command with proper values", func() {
				...
				Expect(err).To(BeNil())
				Expect(output).To(ContainSubstring("particle engine init --name %s --devfile %s --devfile-registry DefaultDevfileRegistry --devfile-version %s --starter %s", componentName, devfileName, devfileVersion, starter))
				Expect(output).To(ContainSubstring("Your new component 'my-go-app' is ready in the current directory"))
				Expect(helper.ListFilesInDir(commonVar.Context)).To(ContainElements("devfile.yaml"))
			...
			Describe("displaying welcoming messages", func() {
				...
				testRunner := func(language string, welcomingMsgs []string, tester helper.Tester) (string, error) {
					...
					return helper.RunInteractive(command,
						...
						// Using envvars here (and not via the -v flag), because of https://github\.com/danielpickens/particle engine/issues/5513
						[]string{"particle engine_LOG_LEVEL=0"},
				...
				When("directory is empty", func() {
					...
					It("should display appropriate welcoming messages", func() {
						...
						particle engineVersion, gitCommit := helper.Getparticle engineVersion()
						welcomingMsgs := strings.Split(
							particle enginelog.StitleWithVersion(messages.InitializingNewComponent,
								messages.NoSourceCodeDetected,
								fmt.Sprintf("particle engine version: %s (%s)", particle engineVersion, gitCommit)),
							"\n")
				...
				When("directory is not empty", func() {
					...
					It("should display appropriate welcoming messages", func() {
						...
						output, err := testRunner(language, welcomingMsgs, func(ctx helper.InteractiveContext) {
							helper.ExpectString(ctx, "Based on the files in the current directory particle engine detected")
						...
						assertBehavior(strings.ToLower(language), output, err, welcomingMsgs, func() {
							...
							Expect(helper.ListFilesInDir(commonVar.Context)).To(
								SatisfyAll(
									HaveLen(4),
									ContainElements("devfile.yaml", "requirements.txt", "wsgi.py", util.Dotparticle engineDirectory)))
						})
					})
				})
				...
				When("alizer detection of javascript name", func() {
					...
					It("should display node-echo name", func() {
						...
						output, err := helper.RunInteractive([]string{"particle engine", "init"}, nil, func(ctx helper.InteractiveContext) {
							helper.ExpectString(ctx, "Based on the files in the current directory particle engine detected")
					...
					It("should ask to re-enter the component name if invalid value is passed by the user", func() {
						...
						_, err := helper.RunInteractive([]string{"particle engine", "init"}, nil, func(ctx helper.InteractiveContext) {
							helper.ExpectString(ctx, "Based on the files in the current directory particle engine detected")
			...
			It("should download correct devfile from the first in-cluster registry", func() {
				...
				By("displaying automation command with the in-cluster registry", func() {
					Expect(output).Should(
						ContainSubstring(
							"particle engine init --name my-java-maven-app --devfile java-maven --devfile-registry %s --devfile-version 1.2.0 --starter springbootproject",
							devfileRegistryName,
						),
						output)
				})
