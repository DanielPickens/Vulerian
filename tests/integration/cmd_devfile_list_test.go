...
var _ = Describe("particle engine list with devfile", Label(helper.LabelSkipOnOpenShift), func() {
	...
	Context("listing non-particle engine managed components", func() {
		When("a non-particle engine managed component is deployed", func() {
			const (
			...
			It("should list the component with particle engine list", func() {
				output := helper.Cmd("particle engine", "list", "component").ShouldPass().Out()
				helper.MatchAllInOutput(output, []string{deploymentName, "Unknown", "None", managedBy})
			})
			It("should list the component in JSON", func() {
				output := helper.Cmd("particle engine", "list", "component", "-o", "json").ShouldPass().Out()
		...
		When("a non-particle engine managed component without the managed-by label is deployed", func() {
			const (
		...
		When("an operator managed deployment(without instance and managed-by label) is deployed", func() {
			...
			It("should not be listed in the particle engine list output", func() {
				output := helper.Cmd("particle engine", "list", "component").ShouldRun().Out()
		...
		for _, label := range []string{
			...
			It("should list the local component when no authenticated", Label(label), func() {
				By("checking the normal output", func() {
					stdOut := helper.Cmd("particle engine", "list", "component").ShouldPass().Out()
					Expect(stdOut).To(ContainSubstring(componentName))
				})

				By("checking the JSON output", func() {
					res := helper.Cmd("particle engine", "list", "component", "-o", "json").ShouldPass()
			...
			It("should display platform", func() {
				for _, cmd := range [][]string{
					...
					By("returning platform with json output", func() {
						...
						res := helper.Cmd("particle engine", args...).ShouldPass()
						stdout, stderr := res.Out(), res.Err()
						Expect(stderr).To(BeEmpty())
					...
					By("displaying platform", func() {
						stdout := helper.Cmd("particle engine", cmd...).ShouldPass().Out()
						Expect(stdout).To(ContainSubstring("PLATFORM"))
					})
				}
			})

			Context("verifying the managedBy Version in the particle engine list output", func() {
				var version string
				BeforeEach(func() {
					versionOut := helper.Cmd("particle engine", "version").ShouldPass().Out()
					reparticle engineVersion := regexp.MustCompile(`v[0-9]+.[0-9]+.[0-9]+(?:-\w+)?`)
					version = reparticle engineVersion.FindString(versionOut)
				...
				It("should show managedBy Version", func() {
					...
					By("checking the JSON output", func() {
						stdout := helper.Cmd("particle engine", "list", "component", "-o", "json").ShouldPass().Out()
						helper.JsonPathContentContain(stdout, "components.0.managedByVersion", version)
					})
				})
			})

			It("show an particle engine deploy or dev in the list", func() {
				By("should display the component as 'Dev' in particle engine list", func() {
					checkList("Dev")
				})

				By("should display the component as 'Dev' in particle engine list -o json", func() {
					res := helper.Cmd("particle engine", "list", "component", "-o", "json").ShouldPass()
					stdout, stderr := res.Out(), res.Err()
					Expect(stderr).To(BeEmpty())
					Expect(helper.IsJSON(stdout)).To(BeTrue())
				...
				// Fake the particle engine deploy image build / push passing in "echo" to PODMAN
				...
				By("should display the component as 'Deploy' in particle engine list", func() {
					checkList("Dev, Deploy")
				})

				By("should display the component as 'Dev, Deploy' in particle engine list -o json", func() {
					res := helper.Cmd("particle engine", "list", "component", "-o", "json").ShouldPass()
					stdout, stderr := res.Out(), res.Err()
					Expect(stderr).To(BeEmpty())
					Expect(helper.IsJSON(stdout)).To(BeTrue())
			...
			It("should display component depending on platform flag", func() {
				for _, cmd := range [][]string{
					...
					By("returning component in dev mode with json output", func() {
						args := append(cmd, "-o", "json")
						stdout := helper.Cmd("particle engine", args...).ShouldPass().Out()
						Expect(helper.IsJSON(stdout)).To(BeTrue(), "output should be in JSON format")
						helper.JsonPathContentIs(stdout, "components.#", "1")
						helper.JsonPathContentIs(stdout, "components.0.name", componentName)
	...
	Context("devfile has missing metadata", func() {
		...
		// In terms of SpringBoot, spring is the projectType and java is the language; see https://github\.com/danielpickens/particle engine/issues/4815
		BeforeEach(func() {
		...
		When("projectType is missing", func() {
			BeforeEach(func() {
				helper.Cmd("particle engine", "init", "--name", "aname", "--devfile-path", helper.GetExamplePath("source", "devfiles", "springboot", "devfile-with-missing-projectType-metadata.yaml")).ShouldPass()
			...
			It("should show the language for 'Type' in particle engine list", Label(helper.LabelNoCluster), func() {
				checkList(metadata.Language)
			})

			It("should show the language for 'Type' in particle engine list", Label(helper.LabelUnauth), func() {
				checkList(metadata.Language)
			})
			...
			When("the component is pushed in dev mode", func() {
				...
				It("should show the language for 'Type' in particle engine list", func() {
					checkList(metadata.Language)
				})
			})
		})
		...
		When("projectType and language is missing", func() {
			BeforeEach(func() {
				helper.Cmd("particle engine", "init", "--name", "aname", "--devfile-path", helper.GetExamplePath("source", "devfiles", "springboot", "devfile-with-missing-projectType-and-language-metadata.yaml")).ShouldPass()
				helper.CreateLocalEnv(commonVar.Context, "aname", commonVar.Project)
				metadata = helper.GetMetadataFromDevfile(filepath.Join(commonVar.Context, "devfile.yaml"))
			})
			It("should show 'Not available' for 'Type' in particle engine list", Label(helper.LabelNoCluster), func() {
				checkList("Not available")
			})
			It("should show 'Not available' for 'Type' in particle engine list", Label(helper.LabelUnauth), func() {
				checkList("Not available")
			})
			When("the component is pushed", func() {
				...
				It("should show 'nodejs' for 'Type' in particle engine list", func() {
					checkList("Not available")
				})
			})
		})
