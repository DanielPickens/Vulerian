...
var _ = Describe("particle engine devfile deploy command tests", func() {
	...
	When("directory is empty", func() {
		...
		It("should error", func() {
			output := helper.Cmd("particle engine", "deploy").ShouldFail().Err()
	...
	When("a component is bootstrapped", func() {
		BeforeEach(func() {
			...
			helper.Cmd("particle engine", "init", "--name", cmpName, "--devfile-path", helper.GetExamplePath("source", "devfiles", "nodejs", "devfile-deploy.yaml")).ShouldPass()
		})
		When("using a default namespace", func() {
			...
			AfterEach(func() {
				helper.Cmd("particle engine", "delete", "component", "-f").ShouldPass()
				commonVar.CliRunner.SetProject(commonVar.Project)
			})

			It("should display warning when running the deploy command", func() {
				errOut := helper.Cmd("particle engine", "deploy").AddEnv("PODMAN_CMD=echo").ShouldRun().Err()
				namespace := "project"
				if helper.IsKubernetesCluster() {
					namespace = "namespace"
				}
				Expect(errOut).To(ContainSubstring(fmt.Sprintf("You are using \"default\" %[1]s, particle engine may not work as expected in the default %[1]s.", namespace)))
			})
		})
		It("should fail to run particle engine deploy when not connected to any cluster", Label(helper.LabelNoCluster), func() {
			errOut := helper.Cmd("particle engine", "deploy").ShouldFail().Err()
	...
	for _, ctx := range []struct {
		...
		When(ctx.title, func() {
			...
			for _, tt := range []struct {
				...
				{
					name: "running particle engine deploy",
				},
				{
					name: "running particle engine deploy with image build extra args",
					imageBuildExtraArgs: []string{
				...
				When(tt.name, func() {
					...
					BeforeEach(func() {
						...
						stdout = helper.Cmd("particle engine", "deploy").AddEnv(env...).ShouldPass().Out()
					})
					...
					It("should run particle engine dev successfully", func() {
					...
					When("running and stopping particle engine dev", func() {
						BeforeEach(func() {
							devSession, err := helper.StartDevMode(helper.DevSessionOpts{})
						...
						It("should not delete the resources created with particle engine deploy", func() {
...
`)
				...
				When("running particle engine deploy", func() {
					var stdout string
					BeforeEach(func() {
						stdout = helper.Cmd("particle engine", "deploy").AddEnv("PODMAN_CMD=echo").ShouldPass().Out()
					})
					It("should succeed", func() {
						By("building and pushing image to registry", func() {
...
`)
						...
						It("should delete the component in the current namespace", func() {
							out := helper.Cmd("particle engine", "delete", "component", "-f").ShouldPass().Out()
	...
	When("using a devfile.yaml containing two deploy commands", func() {
		...
		It("should run particle engine deploy", func() {
			stdout := helper.Cmd("particle engine", "deploy").AddEnv("PODMAN_CMD=echo").ShouldPass().Out()
			By("building and pushing image to registry", func() {
	...
	When("recording telemetry data", func() {
		...
		It("should record the telemetry data correctly", func() {
			...
			Expect(td.Event).To(ContainSubstring("particle engine deploy"))
	...
	When("using a devfile.yaml containing an Image component with a build context", func() {
		...
		BeforeEach(func() {
			helper.CopyExample(filepath.Join("source", "nodejs"), commonVar.Context)
			helper.Cmd("particle engine", "init", "--name", "aname",
				"--devfile-path",
		...
		for _, scope := range []struct {
			...
			It(fmt.Sprintf("should build image via %s if build context references PROJECT_SOURCE env var", scope.name), func() {
				stdout := helper.Cmd("particle engine", "deploy").AddEnv(scope.envvars...).ShouldPass().Out()
		...
		When(fmt.Sprintf("deploying a Devfile %s component with multiple K8s resources defined", ctx.componentType), func() {
			...
			BeforeEach(func() {
				...
				helper.CopyExampleDevFile(
					...
					cmpName)
				out = helper.Cmd("particle engine", "deploy").AddEnv("PODMAN_CMD=echo").ShouldPass().Out()
	...
	When("deploying a ServiceBinding k8s resource", Label(helper.LabelServiceBinding), Label(helper.LabelSkipOnOpenShift), func() {
		...
		When("particle engine deploy is run", func() {
			BeforeEach(func() {
				helper.CopyExample(filepath.Join("source", "nodejs"), commonVar.Context)
				helper.CopyExampleDevFile(
		...
		Describe("using a Devfile with an image component using a remote Dockerfile", func() {
			...
			When("remote server returns an error", func() {
				...
				It("should not build images", func() {
					cmdWrapper := helper.Cmd("particle engine", "deploy").AddEnv(env...).ShouldFail()
...
`)
	...
	Context("deploying devfile with exec", func() {
		...
		for _, ctx := range []struct {
		...
		}{
			{
				...
				compName: "document-how-particle engine-translates-container-component-to-deploymentss",
			},
			{
			...
			When(fmt.Sprintf("using devfile that works; with %s", ctx.title), func() {
				...
				It("should complete the command execution successfully", func() {
					out := helper.Cmd("particle engine", "deploy").ShouldPass().Out()
			...
			// the job name will be truncated, and we want to ensure the correct truncated name is used to delete the old job before running a new one so that `particle engine deploy` does not fail
			When(fmt.Sprintf("the deploy command terminates abruptly; %s", ctx.title), func() {
				BeforeEach(func() {
					...
					helper.Cmd("particle engine", "deploy").WithTimeout(10).ShouldFail()
				})
				When("particle engine deploy command is run again", func() {
					BeforeEach(func() {
			...
			It("should not set securitycontext for podsecurity admission on job's pod template", func() {
				...
				helper.Cmd("particle engine", "deploy").Should(func(session *gexec.Session) {
		...
		When("using a devfile name with length more than 63", func() {
			const (
				unacceptableLongName = "document-how-particle engine-translates-container-component-to-deploymentsss"
			)
			BeforeEach(func() {
		...
		When("using devfile with a long running command in exec", func() {
			...
			It("should print the tip to run particle engine logs after 1 minute of execution", func() {
				out := helper.Cmd("particle engine", "deploy").ShouldPass().Out()
				Expect(out).To(ContainSubstring("Tip: Run `particle engine logs --deploy --follow` to get the logs of the command output."))
			})
		})
		...
		When("using devfile where the exec command is bound to fail", func() {
			...
			It("should print the last 100 lines of the log to the output", func() {
				out, errOut := helper.Cmd("particle engine", "deploy").ShouldFail().OutAndErr()
		...
		When("Automount volumes are present in the namespace", func() {
			...
			It("should mount the volumes", func() {
				helper.Cmd("particle engine", "deploy").Should(func(session *gexec.Session) {
					...
					// More tests are executed on `particle engine dev`, see "Automount volumes are present in the namespace" on particle engine dev tests.
	...
	Context("Devfile with autoBuild or deployByDefault components", func() {
		...
		When("starting with Devfile with Deploy commands", func() {
			...
			When("running particle engine deploy with some components not referenced in the Devfile", func() {
				var stdout string
				...
				It("should create the appropriate resources", func() {
					...
					By("automatically applying image components with autoBuild=true", func() {
						for _, tag := range []string{
							...
							Expect(stdout).Should(ContainSubstring("Building & Pushing Image: localhost:5000/particle engine-dev/node:%s", tag))
						}
					})
					By("automatically applying non-referenced Image components with autoBuild not set", func() {
						Expect(stdout).Should(ContainSubstring("Building & Pushing Image: localhost:5000/particle engine-dev/node:autobuild-not-set-and-not-referenced"))
					})
					By("not applying image components with autoBuild=false", func() {
						for _, tag := range []string{
							...
							Expect(stdout).ShouldNot(ContainSubstring("localhost:5000/particle engine-dev/node:%s", tag))
						}
					})
					By("not applying referenced Image components with deployByDefault unset", func() {
						Expect(stdout).ShouldNot(ContainSubstring("localhost:5000/particle engine-dev/node:autobuild-not-set-and-referenced"))
					})
				})
			})

			When("running particle engine deploy with some components referenced in the Devfile", func() {
				var stdout string

				BeforeEach(func() {
					//Tparticle engine (rm3l): we do not support passing a custom deploy command yet. That's why we are manually updating the Devfile to set the default deploy command.
				...
				It("should create the appropriate resources", func() {
					...
					By("not applying non-referenced image components with autoBuild=false", func() {
						Expect(stdout).ShouldNot(ContainSubstring("localhost:5000/particle engine-dev/node:autobuild-false-and-not-referenced"))
					})
				})
			})
		...
		When("starting with Devfile with no Deploy command", func() {
			...
			It("should fail to run particle engine deploy", func() {
				stdout, stderr := helper.Cmd("particle engine", "deploy").AddEnv("PODMAN_CMD=echo").ShouldFail().OutAndErr()
				...
				By("not automatically applying Image components ", func() {
					for _, s := range []string{stdout, stderr} {
						Expect(s).ShouldNot(ContainSubstring("Building & Pushing Image: localhost:5000/particle engine-dev/node:"))
					}
				})
