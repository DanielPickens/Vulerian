...
var _ = Describe("particle engine create/delete/list/set namespace/project tests", Label(helper.LabelSkipOnOpenShift), func() {
	...
	When("namespace is created with -w", func() {
		// Ref: https://github\.com/danielpickens/particle engine/issues/6827
		var namespace string
		BeforeEach(func() {
			namespace = helper.GenerateProjectName()
			helper.Cmd("particle engine", "create", "namespace", namespace, "--wait").ShouldPass()
		})
		...
		It("should list the new namespace when listing namespace", func() {
			Eventually(func() string {
				return helper.Cmd("particle engine", "list", "namespace").ShouldPass().Out()
	...
	for _, commandName := range []string{"namespace", "project"} {
		...
		Describe("create "+commandName, func() {
			...
			It(fmt.Sprintf("should successfully create the %s", commandName), func() {
				helper.Cmd("particle engine", "create", commandName, namespace, "--wait").ShouldPass()
			...
			It(fmt.Sprintf("should fail to create %s", commandName), func() {
				By("using an existent name", func() {
					helper.Cmd("particle engine", "create", commandName, commonVar.Project).ShouldFail()
				})
				By("using an invalid name", func() {
					helper.Cmd("particle engine", "create", commandName, "12345").ShouldFail()
		...
		Describe("delete "+commandName, func() {
			...
			It("should not succeed to delete a non-existent "+commandName, func() {
				...
				By("using the force flag and asynchronously", func() {
					helper.Cmd("particle engine", "delete", commandName, fakeNamespace, "--force").ShouldFail()
				})

				By("using the force flag and waiting", func() {
					helper.Cmd("particle engine", "delete", commandName, fakeNamespace, "--force", "--wait").ShouldFail()
				})
			})
		...
		Describe("set "+commandName, func() {
			...
			It("should successfully set the "+commandName, func() {
				...
				By(fmt.Sprintf("setting it to a valid %s", commandName), func() {
					...
					helper.Cmd("particle engine", "set", commandName, anotherNs).ShouldPass()
			...
			It(fmt.Sprintf("should not succeed to set the %s", commandName), func() {
				invalidNs := "234567"
				helper.Cmd("particle engine", "set", commandName, invalidNs).ShouldFail()
			...
			When("running inside a component directory", func() {
				...
				BeforeEach(func() {
					...
					// Bootstrap the component with a .particle engine/env/env.yaml file
					particle engineDir := filepath.Join(commonVar.Context, ".particle engine", "env")
					helper.MakeDir(particle engineDir)
					err := helper.CreateFileWithContent(filepath.Join(particle engineDir, "env.yaml"), fmt.Sprintf(`
ComponentSettings:
  Project: %s
`, commonVar.Project))
				...
				It(fmt.Sprintf("should set the %s", commandName), func() {
					...
					By("setting the current active "+commandName, func() {
						...
						cmd := helper.Cmd("particle engine", "set", commandName, activeNs).ShouldPass()
					...
					By("not changing the namespace of the existing component", func() {
						helper.FileShouldContainSubstring(".particle engine/env/env.yaml", "Project: "+commonVar.Project)
					})
				})
			})
		})
		...
		Describe("list "+commandName, func() {
			It("should fail, without cluster", Label(helper.LabelNoCluster), func() {
				out := helper.Cmd("particle engine", "list", commandName).ShouldFail().Err()
			...
			It("should fail, with unauth cluster", Label(helper.LabelUnauth), func() {
				_ = helper.Cmd("particle engine", "list", commandName).ShouldFail()
			})

			It(fmt.Sprintf("should successfully list all the %ss", commandName), func() {
				Eventually(func() string {
					out := helper.Cmd("particle engine", "list", commandName).ShouldPass().Out()
					return out
			...
			It(fmt.Sprintf("should successfully list all the %ss in JSON format", commandName), func() {
				Eventually(func(g Gomega) {
					...
					out := helper.Cmd("particle engine", "list", commandName, "-o", "json").ShouldRun().Out()
