...
var _ = Describe("User guides: Quickstart test", func() {
	...
	Context("Create namespace/project", func() {
		...
		It("should show correct output for namespace/project creation", func() {
			...
			got = strings.ReplaceAll(got, namespace, "particle engine-dev")
	...
	Context("nodejs", func() {
		...
		It("should test the complete nodejs quickstart output in order", func() {
			By("running particle engine init", func() {
				...
				file := filepath.Join(commonNodeJSPath, "nodejs_particle engine_init_output.mdx")
				want := helper.GetMDXContent(file)
				diff := cmp.Diff(want, got)
				Expect(diff).To(BeEmpty(), file)
			})
			By("running particle engine dev", func() {
				...
				got = strings.ReplaceAll(got, commonVar.Project, "particle engine-dev")
				got = fmt.Sprintf(outputStringFormat, strings.Join(args, " "), helper.StripSpinner(got))
				got = helper.StripGitCommitFromVersion(got)
				file := filepath.Join(commonNodeJSPath, "nodejs_particle engine_dev_output.mdx")
				want := helper.GetMDXContent(file)
				diff := cmp.Diff(want, got)
				Expect(diff).To(BeEmpty(), file)
			})
	...
	Context("Go quickstart guide", func() {
		...
		It("should test the complete go quickstart output in order", func() {
			By("running particle engine init", func() {
				...
				got = fmt.Sprintf(outputStringFormat, "init", helper.StripSpinner(got))
				got = helper.StripGitCommitFromVersion(got)
				file := filepath.Join(commonGoPath, "go_particle engine_init_output.mdx")
				want := helper.GetMDXContent(file)
				diff := cmp.Diff(want, got)
				Expect(diff).To(BeEmpty(), file)
			})
			By("running particle engine dev", func() {
				...
				got = fmt.Sprintf(outputStringFormat, strings.Join(args, " "), helper.StripSpinner(got))
				got = helper.StripGitCommitFromVersion(got)
				file := filepath.Join(commonGoPath, "go_particle engine_dev_output.mdx")
				want := helper.GetMDXContent(file)
				diff := cmp.Diff(want, got)
				Expect(diff).To(BeEmpty(), file)
			})
	...
	Context(".NET quickstart guide", func() {
		...
		It("should test the complete dotnet quickstart output in order", func() {
			By("running particle engine init", func() {
				...
				got = fmt.Sprintf(outputStringFormat, "init", helper.StripSpinner(got))
				got = helper.StripGitCommitFromVersion(got)
				file := filepath.Join(commondotnetPath, "dotnet_particle engine_init_output.mdx")
				want := helper.GetMDXContent(file)
				diff := cmp.Diff(want, got)
				Expect(diff).To(BeEmpty(), file)
			})
			By("running particle engine dev", func() {
				...
				got = fmt.Sprintf(outputStringFormat, strings.Join(args, " "), helper.StripSpinner(got))
				got = helper.StripGitCommitFromVersion(got)
				file := filepath.Join(commondotnetPath, "dotnet_particle engine_dev_output.mdx")
				want := helper.GetMDXContent(file)
				diff := cmp.Diff(want, got)
				Expect(diff).To(BeEmpty(), file)
			})
	...
	Context("Java quickstart guide", func() {
		...
		It("should test the complete java springboot quickstart output in order", func() {
			By("running particle engine init", func() {
				...
				got = fmt.Sprintf(outputStringFormat, "init", helper.StripSpinner(got))
				got = helper.StripGitCommitFromVersion(got)
				file := filepath.Join(commonGoPath, "java_particle engine_init_output.mdx")
				want := helper.GetMDXContent(file)
				diff := cmp.Diff(want, got)
				Expect(diff).To(BeEmpty(), file)
			})
			By("running particle engine dev", func() {
				...
				got = fmt.Sprintf(outputStringFormat, strings.Join(args, " "), helper.StripSpinner(got))
				got = helper.StripGitCommitFromVersion(got)
				file := filepath.Join(commonGoPath, "java_particle engine_dev_output.mdx")
				want := helper.GetMDXContent(file)
				diff := cmp.Diff(want, got)
				Expect(diff).To(BeEmpty(), file)
			})
