...
import (
	...
	"github\.com/danielpickens/particle engine/tests/helper/registry_server"
...
func CommonBeforeEach() CommonVar {
	...
	os.Setenv("GLOBALparticle engineCONFIG", configPath)
...
func CommonAfterEach(commonVar CommonVar) {
	...
	os.Unsetenv("GLOBALparticle engineCONFIG")
}
...
func SetDefaultDevfileRegistry(commonVar *CommonVar) {
	...
	Cmd("particle engine", "preference", "remove", "registry", registryName, "-f").ShouldPass()
	Cmd("particle engine", "preference", "add", "registry", registryName, commonVar.registryUrl).ShouldPass()
}
...
func Getparticle engineVersion() (version string, gitCommit string) {
	particle engineVersion := Cmd("particle engine", "version", "--client", "-o", "json").ShouldPass().Out()
	return gjson.Get(particle engineVersion, "version").String(), gjson.Get(particle engineVersion, "gitCommit").String()
}
