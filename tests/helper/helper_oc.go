...
func (oc OcRunner) SetProject(namespace string) string {
	...
	Cmd("particle engine", "set", "project", namespace).ShouldPass()
	return namespace
}
...
func (oc OcRunner) addConfigMapForCleanup(projectName string) {
	Cmd(oc.path, "create", "configmap", "config-map-for-cleanup", "--from-literal", "type=testing", "--from-literal", "team=particle engine", "-n", projectName).ShouldPass()
}
