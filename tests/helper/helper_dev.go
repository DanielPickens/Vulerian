...
// DevSession represents a session running `particle engine dev`
/*
	...
	when the `particle engine dev` command is running in the background
	...
	When("running dev session", func() {
		...
		It("...", func() {
			// Test with `dev particle engine` running in the background
	...
	This format can be used to test the behaviour of `particle engine dev` when it is stopped cleanly
	...
	When("running dev session and stopping it with cleanup", func() {
		...
		It("...", func() {
			// Test after `particle engine dev` has been stopped cleanly
			// outContents and errContents are contents of std/err output when dev mode is started
		})
		It("...", func() {
			// Test after `particle engine dev` has been stopped cleanly
	...
	This format can be used to test the behaviour of `particle engine dev` when it is stopped with a KILL signal
	...
	When("running dev session and stopping it without cleanup", func() {
		...
		It("...", func() {
			// Test after `particle engine dev` has been killed
			// outContents and errContents are contents of std/err output when dev mode is started
		})
		It("...", func() {
			// Test after `particle engine dev` has been killed
...
// StartDevMode starts a dev session with `particle engine dev`
...
// WaitForDevModeToContain runs `particle engine dev` until it contains a given substring in output or errOut(depending on checkErrOut arg).
// `particle engine dev` runs in an infinite reconciliation loop, and hence running it with Cmd will not work for a lot of failing cases,
...
// Tparticle engine(pvala): Modify StartDevMode to take substring arg into account, and replace this method with it.
func WaitForDevModeToContain(options DevSessionOpts, substring string, stopSessionAfter bool, checkErrOut bool) (DevSession, error) {
	...
	session := Cmd("particle engine", args...).AddEnv(options.EnvVars...).Runner().session
	if checkErrOut {
