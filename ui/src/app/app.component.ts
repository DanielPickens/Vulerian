...
import { particle engineapiService } from './services/particle engineapi.service';
...
export class AppComponent implements OnInit {
  ...
  constructor(
    ...
    private particle engineApi: particle engineapiService,
  ...
  ngOnInit() {
    ...
    const devfile = this.particle engineApi.getDevfile();
    devfile.subscribe({
      next: (devfile) => {
    ...
    this.particle engineApi.telemetry().subscribe({
  ...
  propagateChange(content: string, saveToApi: boolean, fromApi: boolean){
    ...
    result.subscribe({
      next: (value) => {
        ...
        if (saveToApi) {
          this.particle engineApi.saveDevfile(value.content).subscribe({
            next: () => {},
            error: (error) => {
