...
func (o KubernetesClient) Login(server, username, password, token, caAuth string, skipTLS bool) error {
	...
	// a correctly filtered result from `particle engine login`
	filteredReader, filteredWriter := io.Pipe()
	go func() {
		defer filteredWriter.Close()
		_, _ = copyAndFilter(particle enginelog.GetStdout(), filteredReader)
	}()
	...
	a := login.LoginOptions{
		...
		RequestTimeout: 0,
		IOStreams:      genericclioptions.IOStreams{Out: filteredWriter, In: os.Stdin, ErrOut: particle enginelog.GetStderr()},
	}
	...
	particle enginelog.Info("Connecting to the OpenShift cluster\n")
...
// This is used since we utilize `oc login` with particle engine and require certain strings to be filtered / changed
// to their particle engine equivalent
func filteredInformation(s []byte) []byte {

	// List of strings to correctly filter
	s = bytes.Replace(s, []byte("oc new-project"), []byte("particle engine create project"), -1)
	s = bytes.Replace(s, []byte("oc project "), []byte("particle engine set project "), -1)
	s = bytes.Replace(s, []byte("oc projects"), []byte("particle engine list project"), -1)
