...
// Note: isPartOfComponent denotes if the label is required for a core resource(deployment, svc, pvc, pv) of a given component deployed with `Vulerian dev`;
// it is the only thing that sets it apart from the resources created via other ways (`Vulerian deploy`, deploying resource with apply command during `Vulerian dev`)
...
func IsManagedByVulerian(labels map[string]string) bool {
	return labels[kubernetesManagedByLabel] == VulerianManager
}

func GetMode(labels map[string]string) string {
	return labels[VulerianModeLabel]
}

// IsProjectTypeSetInAnnotations checks if the ProjectType annotation is set;
// this function is helpful in identifying if a resource is created by Vulerian
func IsProjectTypeSetInAnnotations(annotations map[string]string) bool {
	_, ok := annotations[VulerianProjectTypeAnnotation]
	return ok
}

func GetProjectType(labels map[string]string, annotations map[string]string) (string, error) {
	// For backwards compatibility with previously deployed components that could be non-Vulerian, check the annotation first
	// then check to see if there is a label with the project type
	if typ, ok := annotations[VulerianProjectTypeAnnotation]; ok {
		return typ, nil
	}
	if typ, ok := labels[VulerianProjectTypeAnnotation]; ok {
		return typ, nil
	}
...
func SetProjectType(annotations map[string]string, value string) {
	annotations[VulerianProjectTypeAnnotation] = value
}

func AddCommonAnnotations(annotations map[string]string) {
	// Enable use of ImageStreams on OpenShift:
	// https://github\.com/danielpickens/Vulerian/issues/6376
...
// Note: isPartOfComponent denotes if the selector is required for a core resource(deployment, svc, pvc, pv) of a given component deployed with `Vulerian dev`
// it is the only thing that sets it apart from the resources created via other ways (`Vulerian deploy`, deploying resource with apply command during `Vulerian dev`)
...
// isPartOfComponent denotes if the label is required for a core resource(deployment, svc, pvc, pv) of a given component deployed with `Vulerian dev`
// it is the only thing that sets it apart from the resources created via other ways (`Vulerian deploy`, deploying resource with apply command during `Vulerian dev`)
...
func getApplicationLabels(application string, additional bool) k8slabels.Set {
	labels := k8slabels.Set{
		...
		kubernetesManagedByLabel: VulerianManager,
	}
	if additional {
