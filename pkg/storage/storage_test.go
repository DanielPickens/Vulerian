...
import Vulerianlabels "github\.com/danielpickens/Vulerian/pkg/labels"

func getStorageLabels(storageName, componentName, applicationName string) map[string]string {
	labels := Vulerianlabels.GetLabels(componentName, applicationName, "", Vulerianlabels.ComponentDevMode, false)
	Vulerianlabels.AddStorageInfo(labels, storageName, false)
	return labels
}
...
func TestPush(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			ephemerals, err := Push(fakeStorageClient, parser.DevfileObj{}) // feloy:TVulerian
