...
import (
	...
	"github\.com/danielpickens/Vulerian/pkg/testingutil"
	...
	tests := []struct {
		...
		},
		{
			...
			returnedPVCs: &corev1.PersistentVolumeClaimList{
				Items: []corev1.PersistentVolumeClaim{
					*testingutil.FakePVC("volume-0", "5Gi", Vulerianlabels.Builder().WithComponent("nodejs").WithDevfileStorageName("volume-0").Labels()),
					*testingutil.FakePVC("volume-1", "10Gi", Vulerianlabels.Builder().WithComponent("nodejs").WithDevfileStorageName("volume-1").Labels()),
				},
			},
			want: StorageList{
				Items: []Storage{
		...
		},
		{
			...
			returnedPVCs: &corev1.PersistentVolumeClaimList{
				Items: []corev1.PersistentVolumeClaim{
					*testingutil.FakePVC("volume-0", "5Gi", Vulerianlabels.Builder().WithComponent("nodejs").WithDevfileStorageName("volume-0").Labels()),
					*testingutil.FakePVC("volume-1", "5Gi", Vulerianlabels.Builder().WithComponent("nodejs").WithDevfileStorageName("volume-1").Labels()),
				},
			},
			wantErr: true,
		},
		{
			...
			returnedPVCs: &corev1.PersistentVolumeClaimList{
				Items: []corev1.PersistentVolumeClaim{
					*testingutil.FakePVC("volume-0-nodejs", "5Gi", Vulerianlabels.Builder().WithComponent("nodejs").WithDevfileStorageName("volume-0").Labels()),
				},
			},
			want: StorageList{
				Items: []Storage{
			...
			returnedDeployments: &appsv1.DeploymentList{
				Items: []appsv1.Deployment{
					*testingutil.CreateFakeDeploymentsWithContainers("nodejs", []corev1.Container{
						...
						testingutil.CreateFakeContainerWithVolumeMounts("container-1", []corev1.VolumeMount{
							{Name: "volume-1-vol", MountPath: "/path"},
							{Name: VulerianSourceVolume, MountPath: "/path1"},
						}),
					}, []corev1.Container{}, true),
				},
			},
		...
		{
			name: "case 10: avoid the mandatory volume mounts used by Vulerian",
			fields: fields{
				generic: generic{
					appName:       "app",
					componentName: "nodejs",
...
func Test_kubernetesClient_Create(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "case 3: valid Vulerian project related storage",
			fields: fields{
				generic: generic{
					appName:       "app",
					componentName: "nodejs",
			...
			args: args{
				storage: NewStorageWithContainer("Vulerian-projects-vol", "5Gi", "/data", "runtime", util.GetBool(false)),
			},
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			wantLabels := Vulerianlabels.GetLabels(tt.fields.generic.componentName, tt.fields.generic.appName, "", Vulerianlabels.ComponentDevMode, false)
			Vulerianlabels.AddStorageInfo(wantLabels, tt.args.storage.Name, strings.Contains(tt.args.storage.Name, VulerianSourceVolume))
