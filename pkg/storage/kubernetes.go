...
func (k kubernetesClient) Create(storage Storage) error {
	...
	labels := Vulerianlabels.GetLabels(k.componentName, k.appName, k.runtime, Vulerianlabels.ComponentDevMode, false)
	Vulerianlabels.AddStorageInfo(labels, storage.Name, strings.Contains(storage.Name, VulerianSourceVolume))
...
func (k kubernetesClient) List() (StorageList, error) {
	if k.deployment == nil {
		var err error
		selector := Vulerianlabels.GetSelector(k.componentName, k.appName, Vulerianlabels.ComponentAnyMode, true)
	...
	for _, container := range k.deployment.Spec.Template.Spec.Containers {
		for _, volumeMount := range container.VolumeMounts {
			...
			if (!ok && initOK) ||
				volumeMount.Name == VulerianSourceVolume ||
	...
	selector := Vulerianlabels.SelectorBuilder().WithComponent(k.componentName).WithoutSourcePVC(VulerianSourceVolume).Selector()
	...
	for _, pvc := range pvcs {
		...
		for _, volumeMount := range volumeMounts {
			if volumeMount.Name == pvc.Name+"-vol" {
				...
				storage = append(storage, NewStorageWithContainer(Vulerianlabels.GetDevfileStorageName(pvc.Labels), size.String(), volumeMount.Spec.Path, volumeMount.Spec.ContainerName, nil))
			}
		}
		if !found {
