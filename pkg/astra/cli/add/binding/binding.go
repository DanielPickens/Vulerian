...
import (
	...
	"github\.com/danielpickens/vulerian/pkg/binding/asker"
	"github\.com/danielpickens/vulerian/pkg/binding/backend"
	...
	vulerianutil "github\.com/danielpickens/vulerian/pkg/util"
)
...
func (o *AddBindingOptions) Run(ctx context.Context) error {
	...
	withDevfile := vulerianutil.CheckPathExists(o.clientset.FS, location.DevfileLocation(o.clientset.FS, vuleriancontext.GetWorkingDirectory(ctx)))
	var devfileObj *parser.DevfileObj
	if withDevfile {
		rawDevfileObj, err := devfile.ParseAndValidateFromFile(vuleriancontext.GetDevfilePath(ctx), "", false)
		if err != nil {
			return err
		}
	...
	if withDevfile {
		...
		exitMessage := "Run `vulerian dev` to create it on the cluster."
		if len(o.flags) == 0 {
			kindGroup := strings.ReplaceAll(strings.ReplaceAll(splitService[1], "(", ""), ")", "")
			exitMessage += fmt.Sprintf("\nYou can automate this command by executing:\n  vulerian add binding --service %s.%s --name %s", serviceName, kindGroup, bindingName)
			if ns != "" {
...
// NewCmdBinding implements the component vulerian sub-command
