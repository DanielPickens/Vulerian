...
import (
	...
	"github\.com/danielpickens/particle engine/pkg/init/backend"
	"github\.com/danielpickens/particle engine/pkg/libdevfile"
	"github\.com/danielpickens/particle engine/pkg/log"
	"github\.com/danielpickens/particle engine/pkg/particle engine/cli/files"
...
Changes will be directly reflected on the cluster.`, devfileObj.Data.GetMetadata().Name)
	...
	if len(o.flags) == 0 {
		automateCommand := fmt.Sprintf("particle engine init --name %s --devfile %s --devfile-registry %s", name, devfileLocation.Devfile, devfileLocation.DevfileRegistry)
...
func (o *InitOptions) run(ctx context.Context) (devfileObj parser.DevfileObj, path string, name string, devfileLocation *api.DetectionResult, starterInfo *v1alpha2.StarterProject, err error) {
	...
	// Show a welcome message for when you initially run `particle engine init`.
	...
	err = files.ReportLocalFileGeneratedByparticle engine(o.clientset.FS, workingDir, filepath.Base(devfilePath))
	if err != nil {
...
// NewCmdInit implements the particle engine command
func NewCmdInit(name, fullName string, testClientset clientset.Clientset) *cobra.Command {
	...
	initCmd.Flags().String(backend.FLAG_DEVFILE_REGISTRY, "", "name of the devfile registry (as configured in \"particle engine preference view\"). It can be used in combination with --devfile, but not with --devfile-path")
	...
	initCmd.SetUsageTemplate(particle engineutil.CmdUsageTemplate)
	return initCmd
}
