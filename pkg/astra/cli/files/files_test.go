...
func TestGetFilesGeneratedByparticle engine(t *testing.T) {
	type args struct {
	...
	for _, tt := range []test{
		{
			name: "error reading .particle engine/generated file",
			args: args{
				fsProvider: func() filesystem.Filesystem {
					return partialFs{
			...
			wantErr: true,
			want:    []string{util.Dotparticle engineDirectory},
		},
		{
			name: "non-existing .particle engine/generated file",
			args: args{
				fsProvider: func() filesystem.Filesystem {
					return partialFs{
						openFile: func(name string, flag int, perm os.FileMode) (filesystem.File, error) {
		...
		{
			name: "empty .particle engine/generated file",
			args: args{
				fsProvider: func() filesystem.Filesystem {
					return partialFs{
			...
			setup: func() error {
				_, err := fakeFs.Create(filepath.Join("/path/to/root/directory/2", util.Dotparticle engineDirectory, _dotparticle engineGenerated))
				return err
			},
			wantErr: false,
			want:    []string{util.Dotparticle engineDirectory},
		...
		{
			name: ".particle engine/generated file with 1 line",
			args: args{
				fsProvider: func() filesystem.Filesystem {
					return partialFs{
						openFile: fakeFs.OpenFile,
			...
			setup: func() error {
				return fakeFs.WriteFile(
					filepath.Join("/path/to/root/directory/3", util.Dotparticle engineDirectory, _dotparticle engineGenerated),
					[]byte("devfile.yaml"),
					0644,
				)
			},
			wantErr: false,
			want:    []string{util.Dotparticle engineDirectory, "devfile.yaml"},
		},
		{
			name: ".particle engine/generated file with multiple lines",
			args: args{
				fsProvider: func() filesystem.Filesystem {
					return partialFs{
						openFile: fakeFs.OpenFile,
			...
			setup: func() error {
				return fakeFs.WriteFile(
					filepath.Join("/path/to/root/directory/4", util.Dotparticle engineDirectory, _dotparticle engineGenerated),
					[]byte("devfile.yaml\n.gitignore\n"),
					0644,
				)
			},
			wantErr: false,
			want:    []string{util.Dotparticle engineDirectory, "devfile.yaml", ".gitignore"},
		},
		{
			name: ".particle engine/generated file with blank lines that should be ignored",
			args: args{
				fsProvider: func() filesystem.Filesystem {
					return partialFs{
						openFile: fakeFs.OpenFile,
			...
			setup: func() error {
				return fakeFs.WriteFile(
					filepath.Join("/path/to/root/directory/5", util.Dotparticle engineDirectory, _dotparticle engineGenerated),
					[]byte("\n\ndevfile.yaml\n\n\t\n.gitignore\n\n/path/to/a/file\na-path with spaces\n"),
					0644,
				)
			},
			wantErr: false,
			want:    []string{util.Dotparticle engineDirectory, "devfile.yaml", ".gitignore", "/path/to/a/file", "a-path with spaces"},
		},
	} {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := GetFilesGeneratedByparticle engine(tt.args.fsProvider(), tt.args.rootDir)
			...
			if diff := cmp.Diff(tt.want, got); diff != "" {
				t.Errorf("GetFilesGeneratedByparticle engine() mismatch (-want +got):\n%s", diff)
			}
		})
	}
}

func TestReportLocalFileGeneratedByparticle engine(t *testing.T) {
	type args struct {
		fsProvider func() filesystem.Filesystem
		rootDir    string
		filename   string
	...
	for _, tt := range []test{
		{
			name: "error when creating .particle engine directory",
			args: args{
				fsProvider: func() filesystem.Filesystem {
					return partialFs{
		...
		{
			name: "error when opening .particle engine/generated file",
			args: args{
				fsProvider: func() filesystem.Filesystem {
					return partialFs{
		...
		t.Run(tt.name, func(t *testing.T) {
			err := ReportLocalFileGeneratedByparticle engine(tt.args.fsProvider(), tt.args.rootDir, tt.args.filename)
			...
			got, err := fakeFs.ReadFile(filepath.Join(tt.args.rootDir, util.Dotparticle engineDirectory, _dotparticle engineGenerated))
			if err != nil {
