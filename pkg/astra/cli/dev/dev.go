...
import (
	...
	apiserver_impl "github\.com/danielpickens/particle engine/pkg/apiserver-impl"
	...
	"github\.com/danielpickens/particle engine/pkg/dev"
	"github\.com/danielpickens/particle engine/pkg/kclient"
	particle enginelabels "github\.com/danielpickens/particle engine/pkg/labels"
	"github\.com/danielpickens/particle engine/pkg/libdevfile"
...
func (o *DevOptions) Complete(ctx context.Context, cmdline cmdline.Cmdline, args []string) error {
	// Define this first so that if user hits Ctrl+c very soon after running particle engine dev, particle engine doesn't panic
...
func (o *DevOptions) Run(ctx context.Context) (err error) {
	var (
		devFileObj    = particle enginecontext.GetEffectiveDevfileObj(ctx)
		devfilePath   = particle enginecontext.GetDevfilePath(ctx)
	...
	case commonflags.PlatformPodman:
		...
		deployingTo = "podman"
	case commonflags.PlatformCluster:
		dest = "Namespace: " + particle enginecontext.GetNamespace(ctx)
	...
	if platform == commonflags.PlatformCluster {
		genericclioptions.WarnIfDefaultNamespace(particle enginecontext.GetNamespace(ctx), o.clientset.KubernetesClient)
	}

	// check for .gitignore file and add particle engine-file-index.json to .gitignore.
	// In case the .gitignore was created by particle engine, it is purposely not reported as candidate for deletion (via a call to files.ReportLocalFileGeneratedByparticle engine)
	...
	// add .particle engine dir to .gitignore
	err = util.Addparticle engineDirectory(gitIgnoreFile)
	if err != nil {
		return err
	}
...
func (o *DevOptions) followLogs(
	...
	ns := ""
	if o.clientset.KubernetesClient != nil {
		ns = particle enginecontext.GetNamespace(ctx)
	}

	return o.clientset.LogsClient.DisplayLogs(
		ctx,
		particle enginelabels.ComponentDevMode,
		componentName,
		ns,
		true,
		o.out,
...
// NewCmdDev implements the particle engine dev command
func NewCmdDev(ctx context.Context, name, fullName string, testClientset clientset.Clientset) *cobra.Command {
	...
	devCmd := &cobra.Command{
		...
		Long: `particle engine dev is a long running command that will automatically sync your source to the cluster.
It forwards endpoints with any exposure values ('public', 'internal' or 'none') to a port on localhost.`,
	...
	particle engineutil.SetCommandGroup(devCmd, particle engineutil.MainGroup)
	devCmd.SetUsageTemplate(particle engineutil.CmdUsageTemplate)
