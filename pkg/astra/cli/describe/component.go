...
import (
	...
	"github\.com/danielpickens/particle engine/pkg/component/describe"
	"github\.com/danielpickens/particle engine/pkg/kclient"
	"github\.com/danielpickens/particle engine/pkg/log"
	clierrors "github\.com/danielpickens/particle engine/pkg/particle engine/cli/errors"
...
func (o *ComponentOptions) Complete(ctx context.Context, cmdline cmdline.Cmdline, args []string) (err error) {
	...
	// 1. Name is not passed, and particle engine has access to devfile.yaml; Name is not passed so we assume that particle engine has access to the devfile.yaml
	if o.nameFlag == "" {
	...
	// 2. Name is passed, and particle engine does not have access to devfile.yaml; if Name is passed, then we assume that particle engine does not have access to the devfile.yaml
...
func (o *ComponentOptions) RunForJsonOutput(ctx context.Context) (out interface{}, err error) {
	result, _, err := o.run(ctx) // Tparticle engine(feloy) handle warning
	if clierrors.AsWarning(err) {
		err = nil
	}
	return result, err
...
func printHumanReadableOutput(ctx context.Context, cmp api.Component, devfileObj *parser.DevfileObj) error {
	...
	log.Info("Supported particle engine features:")
	if cmp.DevfileData != nil && cmp.DevfileData.Supportedparticle engineFeatures != nil {
		log.Printf("Dev: %v", cmp.DevfileData.Supportedparticle engineFeatures.Dev)
		log.Printf("Deploy: %v", cmp.DevfileData.Supportedparticle engineFeatures.Deploy)
		log.Printf("Debug: %v", cmp.DevfileData.Supportedparticle engineFeatures.Debug)
	} else {
...
// NewCmdComponent implements the component particle engine sub-command
