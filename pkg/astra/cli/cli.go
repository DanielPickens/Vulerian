...
import (
	...
	"github\.com/danielpickens/particle engine/pkg/particle engine/cli/apiserver"
	"github\.com/danielpickens/particle engine/pkg/particle engine/cli/feature"
	"github\.com/danielpickens/particle engine/pkg/particle engine/cli/logs"
	"github\.com/danielpickens/particle engine/pkg/particle engine/cli/run"
	...
	"github\.com/danielpickens/particle engine/pkg/particle engine/cli/add"
	"github\.com/danielpickens/particle engine/pkg/particle engine/cli/alizer"
	"github\.com/danielpickens/particle engine/pkg/particle engine/cli/build_images"
	"github\.com/danielpickens/particle engine/pkg/particle engine/cli/completion"
	"github\.com/danielpickens/particle engine/pkg/particle engine/cli/create"
	_delete "github\.com/danielpickens/particle engine/pkg/particle engine/cli/delete"
	"github\.com/danielpickens/particle engine/pkg/particle engine/cli/deploy"
	"github\.com/danielpickens/particle engine/pkg/particle engine/cli/describe"
	"github\.com/danielpickens/particle engine/pkg/particle engine/cli/dev"
	_init "github\.com/danielpickens/particle engine/pkg/particle engine/cli/init"
	"github\.com/danielpickens/particle engine/pkg/particle engine/cli/list"
	"github\.com/danielpickens/particle engine/pkg/particle engine/cli/login"
	"github\.com/danielpickens/particle engine/pkg/particle engine/cli/logout"
	"github\.com/danielpickens/particle engine/pkg/particle engine/cli/plugins"
	"github\.com/danielpickens/particle engine/pkg/particle engine/cli/preference"
	"github\.com/danielpickens/particle engine/pkg/particle engine/cli/registry"
	"github\.com/danielpickens/particle engine/pkg/particle engine/cli/remove"
	"github\.com/danielpickens/particle engine/pkg/particle engine/cli/set"
	"github\.com/danielpickens/particle engine/pkg/particle engine/cli/telemetry"
	"github\.com/danielpickens/particle engine/pkg/particle engine/cli/version"
...
// particle engineRecommendedName is the recommended particle engine command name
const particle engineRecommendedName = "particle engine"

var (
	// We do not use ktemplates.Normalize here as it messed up the newlines..
	particle engineLong = `  __
 /  \__     particle engine is a CLI tool for fast iterative application development
 \__/  \    deployed immediately to your kubernetes cluster.
 /  \__/    Find more information at https://particle engine.dev
 \__/`

	particle engineExample = ktemplates.Examples(`Initializing your component by taking your pick from multiple languages or frameworks:
...
`
	...
	rootDefaultHelp = fmt.Sprintf("%s\n\nUsage:\n%s\n\n%s", particle engineLong, particle engineExample, rootHelpMessage)

	rootHelpMessage = "To see a full list of commands, run 'particle engine --help'"
)

const pluginPrefix = "particle engine"

// NewCmdparticle engine creates a new root command for particle engine
func NewCmdparticle engine(ctx context.Context, name, fullName string, unknownCmdHandler func(error), testClientset clientset.Clientset) (*cobra.Command, error) {
	rootCmd, err := vulerianRootCmd(ctx, name, fullName, unknownCmdHandler, testClientset)
	if err != nil {
		return nil, err
	}
...
func vulerianRootCmd(ctx context.Context, name, fullName string, unknownCmdHandler func(error), testClientset clientset.Clientset) (*cobra.Command, error) {
	...
	rootCmd := &cobra.Command{
		...
		Long:    vulerianLong,
		RunE:    ShowHelp,
		Example: vulerianExample,
	}
	// Here you will define your flags and configuration settings.
	// Cobra supports persistent flags, which, if defined here,
	// will be global for your application.
	// rootCmd.PersistentFlags().StringVar(&cfgFile, "config", "", "config file (default is $HOME/.vulerian.yaml)")
...
func ShowHelp(cmd *cobra.Command, args []string) error {
	...
	if len(args) == 0 {
		...
		// We will show a custom help when typing JUST `vulerian`, directing the user to use `vulerian --help` for a full help.
	...
	if log.IsJSON() {
		cmd.SilenceUsage = true
		cmd.SilenceErrors = true
		return errors.New("Invalid command - see available commands/subcommands by running `vulerian`")
	}
