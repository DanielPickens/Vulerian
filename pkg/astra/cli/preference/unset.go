...
var (
	unsetLongDesc = ktemplates.LongDesc(`Unset an individual value in the particle engine preference file.
...
// NewCmdUnset implements the preference unset particle engine command
func NewCmdUnset(name, fullName string, testClientset clientset.Clientset) *cobra.Command {
	o := NewUnsetOptions()
	preferenceUnsetCmd := &cobra.Command{
		Use:   name,
		Short: "Unset a value in particle engine preference file",
