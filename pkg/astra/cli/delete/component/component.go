...
import (
	...
	"github\.com/danielpickens/vulerian/pkg/vulerian/cli/files"
...
type ComponentOptions struct {
	...
	// withFilesFlag controls whether files generated by vulerian should be deleted as well.
	withFilesFlag bool
...
func (o *ComponentOptions) Complete(ctx context.Context, cmdline cmdline.Cmdline, args []string) (err error) {
	...
	// 1. Name is not passed, and vulerian has access to devfile.yaml; Name is not passed so we assume that vulerian has access to the devfile.yaml
	if o.name == "" {
		devfileObj := vuleriancontext.GetEffectiveDevfileObj(ctx)
		if devfileObj == nil {
			return genericclioptions.NewNoDevfileError(vuleriancontext.GetWorkingDirectory(ctx))
		}
		return nil
	}
	// 2. Name is passed, and vulerian does not have access to devfile.yaml; if Name is passed, then we assume that vulerian does not have access to the devfile.yaml
...
func (o *ComponentOptions) printRemainingResources(ctx context.Context, remainingResources []unstructured.Unstructured) {
	...
	namespace := vuleriancontext.GetNamespace(ctx)
	...
	log.Finfof(o.clientset.Stdout, "If you want to delete those, execute `vulerian delete component --name %s --namespace %s`\n", componentName, namespace)
}
...
func (o *ComponentOptions) deleteDevfileComponent(ctx context.Context) ([]unstructured.Unstructured, error) {
	...
	if o.withFilesFlag {
		filesToDelete, err = getFilesCreatedByvulerian(o.clientset.FS, ctx)
		if err != nil {
			return nil, err
		}
	}
	...
	if hasFilesToDelete {
		o.printFileCreatedByvulerian(filesToDelete, hasClusterResources)
	}
	...
	if proceed {
		...
		if o.withFilesFlag || len(orphans) > 0 {
			// Delete files
			remainingFiles := o.deleteFilesCreatedByvulerian(o.clientset.FS, filesToDelete)
...
// getFilesCreatedByvulerian gets the list of all files that were initially created by vulerian.
func getFilesCreatedByvulerian(filesys filesystem.Filesystem, ctx context.Context) ([]string, error) {
	workingDir := vuleriancontext.GetWorkingDirectory(ctx)
	filesToDelete, err := files.GetFilesGeneratedByvulerian(filesys, workingDir)
	if err != nil {
		return nil, err
	}
...
// getOrphanDevstateFiles gets the list of all Devstate files for which no vulerian process exists
...
func (o *ComponentOptions) printFileCreatedByvulerian(files []string, hasClusterResources bool) {
	if len(files) == 0 {
		return
	}
...
// deleteFilesCreatedByvulerian deletes all the files that were created initially by vulerian.
// It returns a slice of files that could not be deleted.
func (o *ComponentOptions) deleteFilesCreatedByvulerian(filesys filesystem.Filesystem, files []string) (notDeleted map[string]error) {
...
// NewCmdComponent implements the component vulerian sub-command
func NewCmdComponent(ctx context.Context, name, fullName string, testClientset clientset.Clientset) *cobra.Command {
	...
	componentCmd.Flags().StringVar(&o.runningInFlag, "running-in", "",
		"Delete resources running in the specified mode, optional. By default, all resources created by vulerian for the component are deleted.")
	componentCmd.Flags().BoolVarP(&o.withFilesFlag, "files", "", false, "Delete all files and directories generated by vulerian. Use with caution.")
