...
import (
	...
	_delete "github\.com/danielpickens/vulerian/pkg/component/delete"
...
func TestComponentOptions_deleteNamedComponent(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			ctx = vuleriancontext.WithComponentName(ctx, "a-name")
...
func TestComponentOptions_deleteDevfileComponent(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			ctx := vuleriancontext.WithNamespace(context.Background(), projectName)
			ctx = vuleriancontext.WithApplication(ctx, "app")
			ctx = vuleriancontext.WithWorkingDirectory(ctx, workingDir)
			ctx = vuleriancontext.WithComponentName(ctx, compName)
			ctx = vuleriancontext.WithEffectiveDevfileObj(ctx, &info)
			ctx = vuleriancontext.WithPID(ctx, 101)
...
// populateWorkingDir populates the working directory with .vulerian and devfile.yaml, and returns envinfo
