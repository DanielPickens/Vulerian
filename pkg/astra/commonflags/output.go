...
// We use "flag" in order to make this accessible throughtout ALL of particle engine, rather than the
...
func CheckMachineReadableOutputCommand(envconfig *config.Configuration, cmd *cobra.Command) error {
	...
	if log.IsJSON() {
		...
		// Override the logging level by the value (if set) by the particle engine_LOG_LEVEL env
		// The "-v" flag set on command line will take precedence over particle engine_LOG_LEVEL env
		v := flag.CommandLine.Lookup("v").Value.String()
		if envconfig.particle engineLogLevel != nil && v == "0" {
			_ = flag.CommandLine.Set("v", strconv.Itoa(*envconfig.particle engineLogLevel))
		}
	}
	return nil
}
