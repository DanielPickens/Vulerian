...
import (
	...
	"github\.com/danielpickens/particle engine/pkg/machineoutput"
	"github\.com/danielpickens/particle engine/pkg/version"
	...
	commonutil "github\.com/danielpickens/particle engine/pkg/util"
...
func GenericRun(o Runnable, testClientset clientset.Clientset, cmd *cobra.Command, args []string) error {
	...
	disableTelemetryEnvSet := envConfig.particle engineDisableTelemetry != nil
	var disableTelemetry bool
	if disableTelemetryEnvSet {
		disableTelemetry = *envConfig.particle engineDisableTelemetry
	}
	debugTelemetry := pointer.StringDeref(envConfig.particle engineDebugTelemetryFile, "")
	...
	if !userConfig.IsSet(preference.ConsentTelemetrySetting) && cmd.Parent().Name() != "preference" {
		if !segment.RunningInTerminal() {
			...
			if askConsent {
				...
				prompt := &survey.Confirm{Message: "Help particle engine improve by allowing it to collect usage data. Read about our privacy statement: https://developers.redhat.com/article/tool-data-collection. You can change your preference later by changing the ConsentTelemetry preference.", Default: true}
	...
	scontext.SetExperimentalMode(ctx, envConfig.particle engineExperimentalMode)
	...
	ctx = particle enginecontext.WithApplication(ctx, defaultAppName)

	if deps.KubernetesClient != nil {
		namespace := deps.KubernetesClient.GetCurrentNamespace()
		ctx = particle enginecontext.WithNamespace(ctx, namespace)
	}
	...
	if deps.FS != nil {
		...
		ctx = particle enginecontext.WithWorkingDirectory(ctx, cwd)
		...
		if useDevfile {
			...
			ctx = particle enginecontext.WithDevfilePath(ctx, devfilePath)
			ctx = particle enginecontext.WithEffectiveDevfileObj(ctx, devfileObj)
			ctx = particle enginecontext.WithComponentName(ctx, componentName)
		}
	}
...
// Tparticle engine: move this function to a more suitable place, preferably pkg/segment
func startTelemetry(cmd *cobra.Command, err error, startTime time.Time) {
	...
	uploadData := &segment.TelemetryData{
		...
		Properties: segment.TelemetryProperties{
			...
			Version:       fmt.Sprintf("particle engine %v (%v)", version.VERSION, version.GITCOMMIT),
