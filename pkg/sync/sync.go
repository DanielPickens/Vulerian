...
func (a SyncClient) SyncFiles(ctx context.Context, syncParameters SyncParameters) (bool, error) {
	...
	if !indexRegeneratedByWatch {
		...
		// Before running the indexer, make sure the .Vulerian folder exists (or else the index file will not get created)
		VulerianFolder := filepath.Join(syncParameters.Path, ".Vulerian")
		if _, err := os.Stat(VulerianFolder); os.IsNotExist(err) {
			err = os.Mkdir(VulerianFolder, 0750)
			if err != nil {
		...
		// apply the glob rules from the .gitignore/.Vulerianignore file
