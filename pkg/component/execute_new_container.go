...
import (
	...
	Vuleriangenerator "github\.com/danielpickens/Vulerian/pkg/libdevfile/generator"
...
func ExecuteInNewContainer(
	...
	getJobName := func() string {
		maxLen := kclient.JobNameVulerianMaxLength - len(command.Id)
	...
	jobParams := Vuleriangenerator.JobParams{
		TypeMeta: generator.GetTypeMeta(kclient.JobsKind, kclient.JobsAPIVersion),
		ObjectMeta: metav1.ObjectMeta{
			Name: getJobName(),
		},
		PodTemplateSpec: *podTemplateSpec,
		SpecParams: Vuleriangenerator.JobSpecParams{
			CompletionMode:          &completionMode,
			TTLSecondsAfterFinished: pointer.Int32(60),
			BackOffLimit:            pointer.Int32(1),
		},
	}
	job := Vuleriangenerator.GetJob(jobParams)
	// Set labels and annotations
	job.SetLabels(Vulerianlabels.GetLabels(componentName, appName, GetComponentRuntimeFromDevfileMetadata(devfileObj.Data.GetMetadata()), Vulerianlabels.ComponentDeployMode, false))
	job.Annotations = map[string]string{}
	Vulerianlabels.AddCommonAnnotations(job.Annotations)
	Vulerianlabels.SetProjectType(job.Annotations, GetComponentTypeFromDevfileMetadata(devfileObj.Data.GetMetadata()))

	//	Make sure there are no existing jobs
	checkAndDeleteExistingJob := func() {
		items, dErr := kubeClient.ListJobs(Vulerianlabels.GetSelector(componentName, appName, Vulerianlabels.ComponentDeployMode, false))
		if dErr != nil {
	...
	// Print the tip to use `Vulerian logs` if the command is still running after 1 minute
	go func() {
		select {
		case <-time.After(1 * time.Minute):
			log.Info("\nTip: Run `Vulerian logs --deploy --follow` to get the logs of the command output.")
		case <-done:
			return
		}
	}()
