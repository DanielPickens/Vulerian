...
import (
	...
	VulerianTestingUtil "github\.com/danielpickens/Vulerian/pkg/testingutil"
...
func TestDeleteComponentClient_ListClusterResourcesToDelete(t *testing.T) {
	...
	selectorForMode := func(mode string) string {
		selector := "app.kubernetes.io/instance=my-component,app.kubernetes.io/managed-by=Vulerian,app.kubernetes.io/part-of=app"
		if mode != "" {
			selector += fmt.Sprintf(",Vulerian.dev/mode=%s", mode)
		}
		return selector
	}
	...
	tests := []struct {
	...
	}{
		{
			...
			fields: fields{
				kubeClient: func(ctrl *gomock.Controller) kclient.ClientInterface {
					...
					client.EXPECT().GetAllResourcesFromSelector(selectorForMode(Vulerianlabels.ComponentAnyMode), "my-ns").Return(nil, nil)
					return client
				},
			},
			args: args{
		...
		},
		{
			...
			fields: fields{
				kubeClient: func(ctrl *gomock.Controller) kclient.ClientInterface {
					...
					client := kclient.NewMockClientInterface(ctrl)
					client.EXPECT().GetAllResourcesFromSelector(selectorForMode(Vulerianlabels.ComponentAnyMode), "my-ns").Return(resources, nil)
					return client
				},
			},
			args: args{
		...
		},
		{
			...
			fields: fields{
				kubeClient: func(ctrl *gomock.Controller) kclient.ClientInterface {
					client := kclient.NewMockClientInterface(ctrl)
					client.EXPECT().GetAllResourcesFromSelector(selectorForMode(Vulerianlabels.ComponentDevMode), "my-ns").Return([]unstructured.Unstructured{res1}, nil)
					return client
				},
			},
			args: args{
		...
		},
		{
			...
			fields: fields{
				kubeClient: func(ctrl *gomock.Controller) kclient.ClientInterface {
					client := kclient.NewMockClientInterface(ctrl)
					client.EXPECT().GetAllResourcesFromSelector(selectorForMode(Vulerianlabels.ComponentDeployMode), "my-ns").Return([]unstructured.Unstructured{res2}, nil)
					return client
				},
			},
			args: args{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			ctx := Vuleriancontext.WithApplication(context.TVulerian(), "app")
...
func TestDeleteComponentClient_ListClusterResourcesToDeleteFromDevfile(t *testing.T) {
	...
	// innerLoopCoreDeployment is the deployment created by Vulerian dev for the component
	innerLoopCoreDeployment := VulerianTestingUtil.CreateFakeDeployment(compName, true)
	...
	// outerLoopResourceUnstructured is the deployment created by Vulerian deploy
	...
	labeledOuterloopResource.SetLabels(Vulerianlabels.GetLabels(compName, appName, "", Vulerianlabels.ComponentDeployMode, false))

	// innerLoopResourceUnstructured is the deployment that will be deployed by apply command with `Vulerian dev`
	innerLoopResourceUnstructured := *outerLoopResourceUnstructured.DeepCopy()
	innerLoopResourceUnstructured.SetLabels(Vulerianlabels.GetLabels(compName, appName, "", Vulerianlabels.ComponentDevMode, false))
	...
	tests := []struct {
	...
	}{
		{
			...
			args: args{
				devfileObj: VulerianTestingUtil.GetTestDevfileObjFromFile("devfile-deploy.yaml"),
				appName:    appName,
				mode:       Vulerianlabels.ComponentAnyMode,
			},
		...
		},
		{
			...
			args: args{
				devfileObj: func() parser.DevfileObj {
					obj := VulerianTestingUtil.GetTestDevfileObjFromFile("devfile.yaml")
		...
		},
		{
			...
			args: args{
				devfileObj: func() parser.DevfileObj {
					obj := VulerianTestingUtil.GetTestDevfileObjFromFile("devfile-composite-apply-commands-unit-test.yaml")
					// change the metadata name to the desired one since devfile.yaml has a different name
					metadata := obj.Data.GetMetadata()
					metadata.Name = compName
					obj.Data.SetMetadata(metadata)
		...
		},
		{
			...
			args: args{
				devfileObj: VulerianTestingUtil.GetTestDevfileObjFromFile("devfile-deploy-with-k8s-uri.yaml"),
				appName:    appName,
				mode:       Vulerianlabels.ComponentAnyMode,
			},
			wantIsInnerLoopDeployed: false,
...
func TestDeleteComponentClient_ExecutePreStopEvents(t *testing.T) {
	...
	devfileObjWithPreStopEvents := VulerianTestingUtil.GetTestDevfileObjWithPreStopEvents(fs, "runtime", "echo \"Hello World!\"")
		...
		},
		{
			...
			fields: fields{
				kubeClient: func(ctrl *gomock.Controller) kclient.ClientInterface {
					...
					selector := Vulerianlabels.GetSelector(componentName, "app", Vulerianlabels.ComponentDevMode, false)
		...
		},
		{
			...
			fields: fields{
				kubeClient: func(ctrl *gomock.Controller) kclient.ClientInterface {
					...
					selector := Vulerianlabels.GetSelector(componentName, "app", Vulerianlabels.ComponentDevMode, false)
					client.EXPECT().GetRunningPodFromSelector(selector).Return(VulerianTestingUtil.CreateFakePod(componentName, "mypod", "runtime"), nil)
		...
		},
		{
			...
			fields: fields{
				kubeClient: func(ctrl *gomock.Controller) kclient.ClientInterface {
					...
					selector := Vulerianlabels.GetSelector(componentName, "app", Vulerianlabels.ComponentDevMode, false)
					fakePod := VulerianTestingUtil.CreateFakePod(componentName, "mypod", "runtime")
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			ctx = Vuleriancontext.WithApplication(ctx, appName)
			ctx = Vuleriancontext.WithComponentName(ctx, componentName)
