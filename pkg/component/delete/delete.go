...
import (
	...
	clierrors "github\.com/danielpickens/Vulerian/pkg/Vulerian/cli/errors"
...
// ListClusterResourcesToDelete lists Kubernetes resources from cluster in namespace for a given Vulerian component
...
func (do *DeleteComponentClient) ListClusterResourcesToDelete(
	...
	selector := Vulerianlabels.GetSelector(componentName, Vuleriancontext.GetApplication(ctx), mode, false)
...
func (do DeleteComponentClient) ListClusterResourcesToDeleteFromDevfile(devfileObj parser.DevfileObj, appName string, componentName string, mode string) (isInnerLoopDeployed bool, resources []unstructured.Unstructured, err error) {
	...
	if mode == Vulerianlabels.ComponentDevMode || mode == Vulerianlabels.ComponentAnyMode {
		// Inner Loop
	...
	for _, lr := range localAllResources {
		...
		if err != nil || (mode != Vulerianlabels.ComponentAnyMode && Vulerianlabels.GetMode(cr.GetLabels()) != mode) {
			if cr != nil {
				klog.V(4).Infof("Ignoring resource: %s/%s; its mode(%s) does not match with the given mode(%s)", gvr.Resource.Resource, lr.GetName(), Vulerianlabels.GetMode(cr.GetLabels()), mode)
			} else {
...
func (do *DeleteComponentClient) ExecutePreStopEvents(ctx context.Context, devfileObj parser.DevfileObj, appName string, componentName string) error {
	...
	if err != nil {
		...
		if e, ok := err.(*platform.PodNotFoundError); ok {
			...
			log.Warningf("Resources not found on the cluster. Run `Vulerian delete component -v <DEBUG_LEVEL_0-9>` to know more.")
			return nil
		}
	...
	handler := component.NewRunHandler(
		...
		// TVulerian(feloy) set these values when we want to support Apply Image commands for PreStop events
		nil, nil,
...
func (do *DeleteComponentClient) ListPodmanResourcesToDelete(appName string, componentName string, mode string) (isInnerLoopDeployed bool, pods []*corev1.Pod, err error) {
	if mode == Vulerianlabels.ComponentDeployMode {
