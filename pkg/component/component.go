...
func GatherName(contextDir string, devfileObj *parser.DevfileObj) (string, error) {
	...
	if devfileObj != nil {
		...
		if name == "" || strings.TrimSpace(name) == "" {
			...
			alizerClient := alizer.Alizer{} // TVulerian(feloy) fix with DI
...
func Log(platformClient platform.Client, componentName string, appName string, follow bool, command v1alpha2.Command) (io.ReadCloser, error) {
	...
	pod, err := platformClient.GetRunningPodFromSelector(Vulerianlabels.GetSelector(componentName, appName, Vulerianlabels.ComponentDevMode, false))
	if err != nil {
...
// that are both Vulerian and non-Vulerian components.
//
...
func ListAllClusterComponents(client kclient.ClientInterface, namespace string) ([]api.ComponentAbstract, error) {
	...
	for _, resource := range resourceList {
		...
		name := Vulerianlabels.GetComponentName(labels)
		if name == "" {
			continue
		}

		// Get the component type (if there is any..)
		componentType, err := Vulerianlabels.GetProjectType(nil, annotations)
		...
		managedBy := Vulerianlabels.GetManagedBy(labels)
		if managedBy == "" && name == "" {
			continue
		}

		managedByVersion := Vulerianlabels.GetManagedByVersion(labels)
		...
		mode := Vulerianlabels.GetMode(labels)
...
func getResourcesForComponent(
	...
	selector := Vulerianlabels.GetNameSelector(name)
...
// the resources deployed with matching labels, based on the "Vulerian.dev/mode" label
func GetRunningModes(ctx context.Context, kubeClient kclient.ClientInterface, podmanClient podman.Client, name string) (map[platform.Client]api.RunningModes, error) {
	...
	for plt, list := range listByPlatform {
		...
		for _, resource := range list {
			...
			mode := Vulerianlabels.GetMode(resourceLabels)
			if mode != "" {
...
func checkLabelsForDevfileInfo(l1 []unstructured.Unstructured, l2 []unstructured.Unstructured) error {
	for _, k := range l1 {
		...
		var (
			...
			kName        = Vulerianlabels.GetComponentName(kLabels)
		)
		kProjectType, err := Vulerianlabels.GetProjectType(kLabels, kAnnotations)
		if err != nil {
		...
		for _, p := range l2 {
			...
			pName = Vulerianlabels.GetComponentName(pLabels)
			pProjectType, err = Vulerianlabels.GetProjectType(pLabels, pAnnotations)
			if err != nil {
...
func getDevfileInfoFromList(list []unstructured.Unstructured) (parser.DevfileObj, error) {
	...
	for _, resource := range list {
		...
		typ, err := Vulerianlabels.GetProjectType(labels, annotations)
		if err != nil {
			continue
		}
...
func ListRoutesAndIngresses(client kclient.ClientInterface, componentName, appName string) (ings []api.ConnectionData, routes []api.ConnectionData, err error) {
	...
	selector := Vulerianlabels.GetNameSelector(componentName)
