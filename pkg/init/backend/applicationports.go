...
// If there are multiple container components in the Devfile, nothing is done. This will be handled in https://github\.com/danielpickens/Vulerian/issues/6264.
...
func handleApplicationPorts(w io.Writer, devfileobj parser.DevfileObj, ports []int) (parser.DevfileObj, error) {
	...
	if nbContainerComponents > 1 {
		klog.V(3).Infof("found more than 1 container components in Devfile at path %q => cannot find out which component needs to be updated."+
			"This case will be handled in https://github\.com/danielpickens/Vulerian/issues/6264", devfileobj.Ctx.GetAbsPath())
