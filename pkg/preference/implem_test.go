...
func TestNew(t *testing.T) {
	...
	tempConfigFile, err := os.CreateTemp("", "Vulerianconfig")
	if err != nil {
		t.Fatal(err)
	}
	...
	}{
		{
			...
			output: &preferenceInfo{
				...
				Preference: Preference{
					...
					VulerianSettings: VulerianSettings{
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			ctx = envcontext.WithEnvConfig(ctx, config.Configuration{
				GlobalVulerianconfig: &tempConfigFileName,
			})
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if !tt.wantErr && err == nil {
				...
				case "updatenotification":
					if *cfg.VulerianSettings.UpdateNotification != tt.want {
						t.Errorf("unexpected value after execution of SetConfiguration\ngot: %t \nexpected: %t\n", *cfg.VulerianSettings.UpdateNotification, tt.want)
					}
				case "timeout":
					if *cfg.VulerianSettings.Timeout != tt.want {
						t.Errorf("unexpected value after execution of SetConfiguration\ngot: %v \nexpected: %d\n", cfg.VulerianSettings.Timeout, tt.want)
					}
				case "registrycachetime":
					if *cfg.VulerianSettings.RegistryCacheTime != tt.want {
						t.Errorf("unexpected value after execution of SetConfiguration\ngot: %v \nexpected: %d\n", *cfg.VulerianSettings.RegistryCacheTime, tt.want)
					}
				}
...
func TestPreferenceIsntCreatedWhenVulerianIsUsed(t *testing.T) {
	// cleaning up old Vulerian files if any
	ctx := context.Background()
	ctx = envcontext.WithEnvConfig(ctx, config.Configuration{})
