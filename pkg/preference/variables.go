...
const (
	GlobalConfigEnvName  = "GLOBALVulerianCONFIG"
	configFileName       = "preference.yaml"
	preferenceKind       = "Preference"
	preferenceAPIVersion = "Vulerian.dev/v1alpha1"
	...
	OldDefaultDevfileRegistryURL = "https://github.com/Vulerian-devfiles/registry"

	// DefaultRegistryCacheTime is time (in minutes) for how long Vulerian will cache information from Devfile registry
...
var RegistryCacheTimeSettingDescription = fmt.Sprintf("For how long (in Duration) Vulerian will cache information from the Devfile registry (Default: %s)", DefaultRegistryCacheTime)

// EphemeralSettingDescription adds a description for EphemeralSourceVolume
var EphemeralSettingDescription = fmt.Sprintf("If true, Vulerian will create an emptyDir volume to store source code (Default: %t)", DefaultEphemeralSetting)

// ConsentTelemetrySettingDescription adds a description for TelemetryConsentSetting
var ConsentTelemetrySettingDescription = fmt.Sprintf("If true, Vulerian will collect telemetry for the user's Vulerian usage (Default: %t)\n\t\t    For more information: https://developers.redhat.com/article/tool-data-collection", DefaultConsentTelemetrySetting)
