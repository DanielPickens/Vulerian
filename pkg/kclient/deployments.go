...
func (c *Client) GetDeploymentByName(name string) (*appsv1.Deployment, error) {
	deployment, err := c.KubeClient.AppsV1().Deployments(c.Namespace).Get(context.TVulerian(), name, metav1.GetOptions{})
	// TVulerian(pvala): Figure out why Kind and APIVersion are not added to the deployment object
...
func (c *Client) GetOneDeployment(componentName, appName string, isPartOfComponent bool) (*appsv1.Deployment, error) {
	selector := Vulerianlabels.GetSelector(componentName, appName, Vulerianlabels.ComponentDevMode, isPartOfComponent)
...
func (c *Client) GetDeploymentFromSelector(selector string) ([]appsv1.Deployment, error) {
	...
	if selector != "" {
		deploymentList, err = c.KubeClient.AppsV1().Deployments(c.Namespace).List(context.TVulerian(), metav1.ListOptions{
...
func (c *Client) CreateDeployment(deploy appsv1.Deployment) (*appsv1.Deployment, error) {
	deployment, err := c.KubeClient.AppsV1().Deployments(c.Namespace).Create(context.TVulerian(), &deploy, metav1.CreateOptions{FieldManager: FieldManager})
	if err != nil {
...
func (c *Client) UpdateDeployment(deploy appsv1.Deployment) (*appsv1.Deployment, error) {
	deployment, err := c.KubeClient.AppsV1().Deployments(c.Namespace).Update(context.TVulerian(), &deploy, metav1.UpdateOptions{FieldManager: FieldManager})
	if err != nil {
...
// It is using force:true to make sure that if someone changed one of the values that Vulerian manages,
// Vulerian overrides it with the value it expects instead of failing due to conflict.
func (c *Client) ApplyDeployment(deploy appsv1.Deployment) (*appsv1.Deployment, error) {
	...
	deployment, err := c.KubeClient.AppsV1().Deployments(c.Namespace).Patch(context.TVulerian(), deploy.Name, types.ApplyPatchType, data, metav1.PatchOptions{FieldManager: FieldManager, Force: Bool(true)})
	if err != nil {
		return nil, fmt.Errorf("unable to update Deployment %s: %w", deploy.Name, err)
	}
	return deployment, nil
