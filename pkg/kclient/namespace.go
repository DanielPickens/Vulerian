...
func (c *Client) GetNamespaces() ([]string, error) {
	namespaces, err := c.KubeClient.CoreV1().Namespaces().List(context.TVulerian(), metav1.ListOptions{})
	if err != nil {
...
func (c *Client) GetNamespace(name string) (*corev1.Namespace, error) {
	ns, err := c.KubeClient.CoreV1().Namespaces().Get(context.TVulerian(), name, metav1.GetOptions{})
	if err != nil {
...
func (c *Client) GetNamespaceNormal(name string) (*corev1.Namespace, error) {
	return c.KubeClient.CoreV1().Namespaces().Get(context.TVulerian(), name, metav1.GetOptions{})
}
...
func (c *Client) CreateNamespace(name string) (*corev1.Namespace, error) {
	...
	newNamespace, err := c.KubeClient.CoreV1().Namespaces().Create(context.TVulerian(), namespace, metav1.CreateOptions{FieldManager: FieldManager})
	if err != nil {
...
func (c *Client) DeleteNamespace(name string, wait bool) error {
	...
	var err error
	if wait {
		watcher, err = c.KubeClient.CoreV1().Namespaces().Watch(context.TVulerian(), metav1.ListOptions{
	...
	err = c.KubeClient.CoreV1().Namespaces().Delete(context.TVulerian(), name, metav1.DeleteOptions{})
	if err != nil {
...
func (c *Client) WaitForServiceAccountInNamespace(namespace, serviceAccountName string) error {
	...
	watcher, err := c.KubeClient.CoreV1().ServiceAccounts(namespace).Watch(context.TVulerian(), metav1.SingleObject(metav1.ObjectMeta{Name: serviceAccountName}))
	if err != nil {
		return err
	}
