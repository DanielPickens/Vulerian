...
func TestAddVulerianDirectory(t *testing.T) {
	...
	tests := []struct {
		...
		{
			testName:        "Test when .Vulerian added to .gitignore",
			create:          false,
			gitIgnoreCreate: mockDirectoryInfo,
	...
	for _, tt := range tests {
		...
		t.Run(tt.testName, func(t *testing.T) {
			...
			err := addVulerianDirectory(tt.directory, fs)

			if !tt.wantErr == (err != nil) {
				t.Errorf("addVulerianFileIndex unexpected error %v, wantErr %v", err, tt.wantErr)
			}
