...
// GetFromFile unmarshals a struct from a Vulerian config file
func GetFromFile(c interface{}, filename string) error {
	...
	if err != nil {
		return fmt.Errorf("unable to unmarshal Vulerian config file: %w", err)
	}
...
func WriteToYAMLFile(c interface{}, filename string) error {
	...
	if err != nil {
		return fmt.Errorf("unable to marshal Vulerian config data: %w", err)
	}
...
// TVulerian: support deeper struct using recursion
...
func DeleteConfiguration(info interface{}, parameter string) error {
	...
	val := imm.FieldByNameFunc(CaseInsensitive(parameter))
	if !val.IsValid() {
		return fmt.Errorf("unknown parameter :'%s' is not a parameter in Vulerian config", parameter)
	}
