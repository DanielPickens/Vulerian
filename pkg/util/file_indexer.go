...
const DotVulerianDirectory = ".Vulerian"
const fileIndexName = "Vulerian-file-index.json"
...
// ReadFileIndex tries to read the Vulerian index file from the given location and returns the data from the file
...
func ReadFileIndex(filePath string) (*FileIndex, error) {
	...
	if err != nil {
		...
		// TVulerian: we need to remove this later
		return NewFileIndex(), nil
	}
	return &fi, nil
}

// ResolveIndexFilePath resolves the filepath of the Vulerian index file in the .Vulerian folder
func ResolveIndexFilePath(directory string) (string, error) {
	...
	case mode.IsDir():
		// do directory stuff
		return filepath.Join(directory, DotVulerianDirectory, fileIndexName), nil
	case mode.IsRegular():
		// do file stuff
		// for binary files
		return filepath.Join(filepath.Dir(directory), DotVulerianDirectory, fileIndexName), nil
	}

	return directory, nil
}

// GetIndexFileRelativeToContext returns the index file relative to context i.e.; .Vulerian/Vulerian-file-index.json
func GetIndexFileRelativeToContext() string {
	return filepath.Join(DotVulerianDirectory, fileIndexName)
}

// AddVulerianFileIndex adds Vulerian-file-index.json to .gitignore
func AddVulerianDirectory(gitIgnoreFile string) error {
	return addVulerianDirectory(gitIgnoreFile, filesystem.DefaultFs{})
}

func addVulerianDirectory(gitIgnoreFile string, fs filesystem.Filesystem) error {
	return addFileToIgnoreFile(gitIgnoreFile, DotVulerianDirectory, fs)
}
...
func recursiveChecker(pathOptions recursiveCheckerPathOptions, ignoreRules []string, remoteDirectories map[string]string, existingFileIndex FileIndex) (IndexerRet, error) {
	...
	for _, matchedPath := range matchedPathsDir {
		...
		if stat.IsDir() {
			...
			if stat.Name() == DotVulerianDirectory {
				return IndexerRet{}, nil
			}
