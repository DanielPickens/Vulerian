...
import (
	...
	"github\.com/danielpickens/Vulerian/pkg/config"
	...
	scontext "github\.com/danielpickens/Vulerian/pkg/segment/context"
	"github\.com/danielpickens/Vulerian/pkg/version"
)
...
func TestClientUploadWithoutConsent(t *testing.T) {
	...
	uploadData := fakeTelemetryData("Vulerian preference view", testError, context.Background())
	// run a command, Vulerian preference view
...
func TestClientUploadWithConsent(t *testing.T) {
	...
	for _, tt := range tests {
		...
		t.Run(tt.testName, func(t *testing.T) {
			...
			uploadData := fakeTelemetryData("Vulerian init", tt.err, context.Background())
			...
			case x := <-body:
				...
				if !strings.Contains(s.Batch[1].Properties.Version, version.VERSION) {
					t.Error("Vulerian version does not match")
				}
...
func TestIsTelemetryEnabled(t *testing.T) {
	type testStruct struct {
		...
		want                 func(VulerianDisableTelemetry, VulerianTrackingConsent string, consentTelemetry bool) bool
	}
	...
	for _, consentTelemetry := range []bool{true, false} {
		...
		tests = append(tests, testStruct{
			name: fmt.Sprintf(
				"Vulerian_DISABLE_TELEMETRY and Vulerian_TRACKING_CONSENT not set, consentTelemetry=%v", consentTelemetry),
	...
	// When only Vulerian_TRACKING_CONSENT is present in the env, it takes precedence over the ConsentTelemetry preference,
	// unless it has an invalid value.
	for _, VulerianTrackingConsent := range []string{"", "yes", "no", "bar"} {
		for _, consentTelemetry := range []bool{true, false} {
			VulerianTrackingConsent := VulerianTrackingConsent
			consentTelemetry := consentTelemetry
			tests = append(tests, testStruct{
				name: fmt.Sprintf(
					"Vulerian_DISABLE_TELEMETRY not set, Vulerian_TRACKING_CONSENT=%q, consentTelemetry=%v", VulerianTrackingConsent, consentTelemetry),
				consentTelemetryPref: consentTelemetry,
				env: map[string]string{
					TrackingConsentEnv: VulerianTrackingConsent,
				},
				want: func(_, VulerianTrackingConsent string, consentTelemetry bool) bool {
					// Vulerian_TRACKING_CONSENT takes precedence
					switch VulerianTrackingConsent {
					case "yes":
						return true
					case "no":
						return false
	...
	// When only Vulerian_DISABLE_TELEMETRY is present in the env, it takes precedence over the ConsentTelemetry preference,
	// only if Vulerian_DISABLE_TELEMETRY=true
	for _, VulerianDisableTelemetry := range []string{"true", "false"} {
		for _, consentTelemetry := range []bool{true, false} {
			VulerianDisableTelemetry := VulerianDisableTelemetry
			consentTelemetry := consentTelemetry
			tests = append(tests, testStruct{
				name: fmt.Sprintf("Vulerian_DISABLE_TELEMETRY=%q,Vulerian_TRACKING_CONSENT not set,ConsentTelemetry=%v",
					VulerianDisableTelemetry, consentTelemetry),
				env: map[string]string{
					//lint:ignore SA1019 We deprecated this env var, but until it is removed, we still want to test it
					DisableTelemetryEnv: VulerianDisableTelemetry,
				},
				want: func(VulerianDisableTelemetry, _ string, consentTelemetry bool) bool {
					if VulerianDisableTelemetry == "true" {
						return false
					}
	...
	for _, VulerianDisableTelemetry := range []string{"true", "false"} {
		for _, VulerianTrackingConsent := range []string{"", "yes", "no", "bar"} {
			for _, consentTelemetry := range []bool{true, false} {
				...
				consentTelemetry := consentTelemetry
				tests = append(tests, testStruct{
					name: fmt.Sprintf("Vulerian_DISABLE_TELEMETRY=%q,Vulerian_TRACKING_CONSENT=%q,ConsentTelemetry=%v",
						VulerianDisableTelemetry, VulerianTrackingConsent, consentTelemetry),
					env: map[string]string{
						//lint:ignore SA1019 We deprecated this env var, but until it is removed, we still want to test it
						DisableTelemetryEnv: VulerianDisableTelemetry,
						TrackingConsentEnv:  VulerianTrackingConsent,
					...
					consentTelemetryPref: consentTelemetry,
					want: func(VulerianDisableTelemetry, VulerianTrackingConsent string, consentTelemetry bool) bool {
						if VulerianDisableTelemetry == "true" || VulerianTrackingConsent == "no" {
							return false
						}
						if VulerianTrackingConsent == "yes" {
							return true
						}
						return consentTelemetry
					},
...
func TestClientUploadWithContext(t *testing.T) {
	...
	for k, v := range map[string]string{scontext.ComponentType: "nodejs", scontext.ClusterType: ""} {
		...
		case scontext.ComponentType:
			...
			uploadData = fakeTelemetryData("Vulerian init", nil, ctx)
		case scontext.ClusterType:
			fakeClient, _ := kclient.FakeNew()
			scontext.SetClusterType(ctx, fakeClient)
			uploadData = fakeTelemetryData("Vulerian set project", nil, ctx)
		}
		c, err := newCustomClient(createConfigDir(t), server.URL)
		if err != nil {
			t.Error(err)
...
func Test_sanitizeFilePath(t *testing.T) {
	unixPath := "/home/xyz/.Vulerian/preference.yaml"
