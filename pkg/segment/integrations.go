...
// getTelemetryForDevfileRegistry returns a populated TelemetryData object that contains some Vulerian telemetry (with client consent), such as the anonymous ID and
// locale in addition to the generic client name "Vulerian"
func getTelemetryForDevfileRegistry(ctx context.Context) (registryLibrary.TelemetryData, error) {
	...
	if envConfig.VulerianDebugTelemetryFile != nil {
		return td, nil
	}
