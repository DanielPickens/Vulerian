...
const TelemetryClient = "Vulerian"

// DisableTelemetryEnv is name of environment variable, if set to true it disables Vulerian telemetry completely
...
const (
	...
	DisableTelemetryEnv = "Vulerian_DISABLE_TELEMETRY"
	// TrackingConsentEnv controls whether Vulerian tracks telemetry or not.
	...
	TrackingConsentEnv = "Vulerian_TRACKING_CONSENT"
)
...
type Client struct {
	...
	// TelemetryFilePath points to the file containing anonymousID used for tracking Vulerian commands executed by the user
...
func SetError(err error) (errString string) {
	...
	// Sanitize exec commands: For errors when a command exec fails in cases like Vulerian exec or Vulerian test, we do not want to know the command that the user executed, so we simply return
...
// RunningInTerminal checks if Vulerian was run from a terminal
...
func IsTelemetryEnabled(cfg preference.Client, envConfig config.Configuration) bool {
	...
	disableTelemetry := pointer.BoolDeref(envConfig.VulerianDisableTelemetry, false)
	if disableTelemetry {
		//lint:ignore SA1019 We deprecated this env var, but until it is removed, we still need to support it
...
func IsTrackingConsentEnabled(envConfig *config.Configuration) (value string, enabled bool, present bool, err error) {
	if envConfig.VulerianTrackingConsent == nil {
		return "", false, false, nil
	}
	trackingConsent := *envConfig.VulerianTrackingConsent
...
// sanitizeExec sanitizes commands from the error string that might have been executed by users while running commands like Vulerian test or Vulerian exec
