...
func TestGetDevfileRegistries(t *testing.T) {
	...
	_, err = tempConfigFile.Write([]byte(
		`kind: Preference
apiversion: Vulerian.openshift.io/v1alpha1
VulerianSettings:
  RegistryList:
...
func TestListDevfileStacks(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// TVulerian(rm3l) Test with both nil and non-nil kubeclient
	...
	tests := []test{
		{
			...
			registryServerProvider: func(t *testing.T) (*httptest.Server, string) {
				return nil, "https://github\.com/danielpickens/Vulerian"
			},
			wantErr: true,
		},
		{
			...
			registryServerProvider: func(t *testing.T) (*httptest.Server, string) {
				return nil, "https://raw.githubusercontent.com/daniel-pickens/Vulerian"
			},
			wantErr: true,
		},
		{
			name: "GitHub-based registry: *.github.com",
			registryServerProvider: func(t *testing.T) (*httptest.Server, string) {
				return nil, "https://daniel-pickens.github.com/Vulerian"
			},
			wantErr: true,
		},
		{
			name: "GitHub-based registry: *.raw.githubusercontent.com",
			registryServerProvider: func(t *testing.T) (*httptest.Server, string) {
				return nil, "https://daniel-pickens.raw.githubusercontent.com/Vulerian"
			},
			wantErr: true,
		},
		{
