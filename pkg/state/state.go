...
import (
	...
	"github\.com/danielpickens/Vulerian/pkg/Vulerian/cli/feature"
...
func (o *State) Init(ctx context.Context) error {
	var (
		pid      = Vuleriancontext.GetPID(ctx)
...
func (o *State) SetForwardedPorts(ctx context.Context, fwPorts []api.ForwardedPort) error {
	...
	// TVulerian(feloy) When other data is persisted into the state file, it will be needed to read the file first
...
func (o *State) writeStateFile(path string) error {
	...
	// .Vulerian directory is supposed to exist, don't create it
...
func (o *State) checkFirstInPlatform(ctx context.Context) error {
	...
	for _, entry := range entries {
		...
		if exists {
			...
			if process.Executable() != "Vulerian" && process.Executable() != "Vulerian.exe" {
				klog.V(4).Infof("process %d exists but is not Vulerian, ignoring", content.PID)
				continue
			}
...
func (o *State) GetOrphanFiles(ctx context.Context) ([]string, error) {
	...
	for _, entry := range entries {
		...
		if exists {
			...
			if process.Executable() != "Vulerian" && process.Executable() != "Vulerian.exe" {
				klog.V(4).Infof("process %d exists but is not Vulerian => orphan", content.PID)
				result = append(result, filename)
				continue
			}
			// Process exists => not orphan
			klog.V(4).Infof("process %d exists and is Vulerian => not orphan", content.PID)
			continue
		}
		klog.V(4).Infof("process %d does not exist => orphan", content.PID)
		result = append(result, filename)
