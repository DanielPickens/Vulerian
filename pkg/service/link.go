...
func pushLinksWithoutOperator(client kclient.ClientInterface, u unstructured.Unstructured, labels map[string]string) error {
	...
	for _, service := range services {
		serviceCompMap[service.Name] = Vulerianlabels.GetComponentName(service.Labels)
	}
	...
	deployment, err := client.GetOneDeploymentFromSelector(Vulerianlabels.GetSelector(Vulerianlabels.GetComponentName(labels), Vulerianlabels.GetAppName(labels), Vulerianlabels.ComponentAnyMode, true))
	if err != nil {
		return err
	}
...
func getPipeline(client kclient.ClientInterface) (sboPipeline.Pipeline, error) {
	...
	return VulerianDefaultBuilder.WithContextProvider(
		sboContext.Provider(
