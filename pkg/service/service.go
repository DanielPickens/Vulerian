...
// TVulerian: make it unlink the service from component as a part of
// https://github\.com/danielpickens/Vulerian/issues/3563
...
func PushKubernetesResource(client kclient.ClientInterface, u unstructured.Unstructured, labels map[string]string, annotations map[string]string, mode string) error {
	...
	if isLinkResource(u.GetKind()) && mode == Vulerianlabels.ComponentDevMode && !sboSupported {
...
func ListDeployedServices(client kclient.ClientInterface, labels map[string]string) (map[string]DeployedInfo, error) {
	...
	for _, svc := range deployedServices {
		...
		if Vulerianlabels.IsManagedByVulerian(deployedLabels) && Vulerianlabels.GetComponentName(deployedLabels) == Vulerianlabels.GetComponentName(labels) {
