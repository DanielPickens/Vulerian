...
import (
	...
	"github\.com/danielpickens/Vulerian/pkg/dev/kubedev/storage"
	"github\.com/danielpickens/Vulerian/pkg/dev/kubedev/utils"
	...
	"github\.com/danielpickens/Vulerian/pkg/service"
	storagepkg "github\.com/danielpickens/Vulerian/pkg/storage"
...
func (o *DevClient) createComponents(ctx context.Context, parameters common.PushParameters, componentStatus *watch.ComponentStatus) (bool, error) {
	...
	// Set the mode to Dev since we are using "Vulerian dev" here
	runtime := component.GetComponentRuntimeFromDevfileMetadata(parameters.Devfile.Data.GetMetadata())
	labels := Vulerianlabels.GetLabels(componentName, appName, runtime, Vulerianlabels.ComponentDevMode, false)
	...
	selector := Vulerianlabels.GetSelector(componentName, appName, Vulerianlabels.ComponentDevMode, false)
	...
	// and the service binding secrets must be deleted manually since they are created by Vulerian
	...
	_, err = o.pushDevfileKubernetesComponents(ctx, parameters, labels, Vulerianlabels.ComponentDevMode, ownerReference)
	if err != nil {
		return false, err
	}
...
func (o *DevClient) getComponentDeployment(ctx context.Context) (*appsv1.Deployment, bool, error) {
	...
	// Since `Vulerian deploy` can theoretically deploy a deployment as well with the same instance name
	// we make sure that we are retrieving the deployment with the Dev mode, NOT Deploy.
	selectorLabels := Vulerianlabels.GetSelector(componentName, appName, Vulerianlabels.ComponentDevMode, true)
...
func (o *DevClient) createOrUpdateComponent(
	...
	// Set the labels
	labels := Vulerianlabels.GetLabels(componentName, appName, runtime, Vulerianlabels.ComponentDevMode, true)

	annotations := make(map[string]string)
	Vulerianlabels.SetProjectType(annotations, component.GetComponentTypeFromDevfileMetadata(parameters.Devfile.Data.GetMetadata()))
	Vulerianlabels.AddCommonAnnotations(annotations)
...
func (o DevClient) getRemoteResourcesNotPresentInDevfile(ctx context.Context, parameters common.PushParameters, selector string) (objectsToRemove, serviceBindingSecretsToRemove []unstructured.Unstructured, err error) {
	...
	for _, remoteK := range allRemoteK8sResources {
		if !Vulerianlabels.IsCoreComponent(remoteK.GetLabels()) {
			// ignore the resources that are already set for deletion
			// ignore the resources that do not have projecttype annotation set; they will be the resources that are not created by Vulerian
			...
			if remoteK.GetDeletionTimestamp() != nil && !Vulerianlabels.IsProjectTypeSetInAnnotations(remoteK.GetAnnotations()) {
				continue
			}
...
func (o *DevClient) pushDevfileKubernetesComponents(
	...
	// from Vulerian standpoint, these components contain yaml manifest of ServiceBinding
...
func (o *DevClient) buildVolumes(ctx context.Context, parameters common.PushParameters, containers, initContainers []corev1.Container) ([]corev1.Volume, error) {
	...
	err := storage.HandleVulerianSourceStorage(o.kubernetesClient, storageClient, componentName, o.prefClient.GetEphemeralSourceVolume())
	if err != nil {
		return nil, err
	}
	...
	// These PVCs have been created earlier with `storage.HandleVulerianSourceStorage` and `storagepkg.Push`
	pvcs, err := o.kubernetesClient.ListPVCs(fmt.Sprintf("%v=%v", "component", componentName))
	if err != nil {
		return nil, err
	}
	...
	// VulerianSourcePVCName will be empty when Ephemeral preference is true
	VulerianSourcePVCName, volumeNameToVolInfo, err := storage.GetVolumeInfos(pvcs)
	if err != nil {
		return nil, err
	}

	// Add the volumes for the projects source and the Vulerian-specific directory
	VulerianMandatoryVolumes := utils.GetVulerianContainerVolumes(VulerianSourcePVCName)
	allVolumes = append(allVolumes, VulerianMandatoryVolumes...)

	// Add the volumeMounts for the project sources volume and the Vulerian-specific volume into the containers
	utils.AddVulerianProjectVolume(containers)
	utils.AddVulerianMandatoryVolume(containers)
