...
import (
	...
	"github\.com/danielpickens/Vulerian/pkg/storage"
...
// GetVolumeInfos returns the PVC name attached to the `Vulerian-projects` directory and a map of other PVCs
func GetVolumeInfos(pvcs []corev1.PersistentVolumeClaim) (VulerianSourcePVCName string, infos map[string]VolumeInfo, _ error) {
	...
	for _, pvc := range pvcs {
		...
		storageName := Vulerianlabels.GetStorageName(pvc.Labels)
		if storageName == storage.VulerianSourceVolume {
			VulerianSourcePVCName = pvc.Name
			continue
		}
	...
	return VulerianSourcePVCName, infos, nil
}
...
// HandleVulerianSourceStorage creates or deletes the volume containing project sources, based on the preference setting
// - if Ephemeral preference is true, any PVC with labels "component=..." and "Vulerian-source-pvc=Vulerian-projects" is removed
// - if Ephemeral preference is false and no PVC with matching labels exists, it is created
func HandleVulerianSourceStorage(client kclient.ClientInterface, storageClient storage.Client, componentName string, isEphemeral bool) error {
	selector := Vulerianlabels.Builder().WithComponentName(componentName).WithSourcePVC(storage.VulerianSourceVolume).Selector()
		...
		if len(pvcs) == 0 {
			err := storageClient.Create(storage.Storage{
				ObjectMeta: metav1.ObjectMeta{
					Name: storage.VulerianSourceVolume,
				},
				Spec: storage.StorageSpec{
					Size: storage.VulerianSourceVolumeSize,
				},
			})
