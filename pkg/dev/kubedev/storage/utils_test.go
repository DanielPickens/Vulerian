...
func TestGetVolumeInfos(t *testing.T) {
	tests := []struct {
		...
		wantVulerianSourcePVCName string
		wantInfos            map[string]VolumeInfo
		wantErr              bool
	}{
		{
			name: "Vulerian-projects is not found",
			pvcs: []corev1.PersistentVolumeClaim{
				{
		...
		{
			name: "Vulerian-projects is found",
			pvcs: []corev1.PersistentVolumeClaim{
				{
					ObjectMeta: metav1.ObjectMeta{
						Name: "pvc1",
			...
			wantVulerianSourcePVCName: "pvc1",
			wantInfos:            map[string]VolumeInfo{},
		},
		{
			name: "Vulerian-projects is found and other pvcs",
			pvcs: []corev1.PersistentVolumeClaim{
				{
					ObjectMeta: metav1.ObjectMeta{
						Name: "pvc1",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			VulerianSourcePVCName, infos, err := GetVolumeInfos(tt.pvcs)
			...
			if diff := cmp.Diff(tt.wantVulerianSourcePVCName, VulerianSourcePVCName); diff != "" {
				t.Errorf("GetVolumeInfos() wantVulerianSourcePVCName mismatch (-want +got):\n%s", diff)
			}
