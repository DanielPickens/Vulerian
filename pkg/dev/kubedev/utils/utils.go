...
// GetVulerianContainerVolumes returns the mandatory Kube volumes for an Vulerian component
func GetVulerianContainerVolumes(sourcePVCName string) []corev1.Volume {
	...
	return []corev1.Volume{
		sourceVolume,
		{
			...
			// in order to pass over some files for Vulerian
...
// AddVulerianProjectVolume adds the Vulerian project volume to the containers
func AddVulerianProjectVolume(containers []corev1.Container) {
	if containers == nil {
		return
	}
...
// AddVulerianMandatoryVolume adds the Vulerian mandatory volumes to the containers
func AddVulerianMandatoryVolume(containers []corev1.Container) {
	if containers == nil {
		return
	}
	for i := range containers {
...
func overrideContainerCommandAndArgsIfNeeded(container *corev1.Container) {
	...
	// TVulerian(rm3l): Remove this once https://github.com/devfile/registry/pull/102 is merged on the Devfile side
