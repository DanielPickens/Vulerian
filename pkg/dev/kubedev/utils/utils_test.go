...
func TestAddVulerianProjectVolume(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			AddVulerianProjectVolume(tt.containers)
			...
			for wantContainerName, wantMountPath := range tt.volMount {
				...
				for _, container := range tt.containers {
					if container.Name == wantContainerName {
						for _, volMount := range container.VolumeMounts {
							if volMount.Name == storage.VulerianSourceVolume && volMount.MountPath == wantMountPath {
								matched = true
							}
						}
					}
				...
				if !matched {
					t.Error("TestAddVulerianProjectVolume error: did not match the volume mount for Vulerian-projects")
				}
			}
		})
	}
...
func TestAddVulerianMandatoryVolume(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			AddVulerianMandatoryVolume(tt.containers)
