...
func TestCreateOrUpdateComponent(t *testing.T) {
	...
	deployment := v1.Deployment{
		...
		ObjectMeta: metav1.ObjectMeta{
			...
			Labels:      Vulerianlabels.Builder().WithComponentName(testComponentName).WithAppName(testAppName).Labels(),
			Annotations: Vulerianlabels.Builder().WithProjectType("").Labels(),
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.componentType != "" {
				Vulerianlabels.SetProjectType(deployment.Annotations, string(tt.componentType))
				comp = VulerianTestingUtil.GetFakeContainerComponent("component")
			}
			...
			ctx = Vuleriancontext.WithApplication(ctx, "app")
			ctx = Vuleriancontext.WithComponentName(ctx, "my-component")
			ctx = Vuleriancontext.WithDevfilePath(ctx, "/path/to/devfile")
	...
	tests := []struct {
	...
	}{
		{
			...
			fields: fields{
				...
				appName:       "app",
				deployment:    VulerianTestingUtil.CreateFakeDeployment("nodejs", false),
			},
			args: args{
				labels:      VulerianTestingUtil.CreateFakeDeployment("nodejs", false).Labels,
				annotations: nil,
			},
			want:    generator.GetObjectMeta("nodejs", "project-0", VulerianTestingUtil.CreateFakeDeployment("nodejs", false).Labels, nil),
			wantErr: false,
		},
		{
			...
			want:    generator.GetObjectMeta(namespacedKubernetesName, "project-0", VulerianTestingUtil.CreateFakeDeployment("nodejs", false).Labels, nil),
			wantErr: false,
		},
		{
			...
			args: args{
				labels:      VulerianTestingUtil.CreateFakeDeployment("nodejs", false).Labels,
				annotations: Vulerianlabels.Builder().WithMode(Vulerianlabels.ComponentDevMode).Labels(),
			},
			want:    generator.GetObjectMeta("nodejs", "project-0", VulerianTestingUtil.CreateFakeDeployment("nodejs", false).Labels, Vulerianlabels.Builder().WithMode(Vulerianlabels.ComponentDevMode).Labels()),
			wantErr: false,
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			ctx := context.Background()
			ctx = Vuleriancontext.WithApplication(ctx, "app")
			ctx = Vuleriancontext.WithComponentName(ctx, "nodejs")
			ctx = Vuleriancontext.WithDevfilePath(ctx, "/path/to/devfile")
