...
import (
	...
	"github\.com/danielpickens/Vulerian/pkg/libdevfile/generator"
	...
	basePod := corev1.Pod{
		...
		ObjectMeta: v1.ObjectMeta{
			...
			Labels: map[string]string{
				...
				"Vulerian.dev/mode":                         labels.ComponentDevMode,
				"Vulerian.dev/project-type":                 "Not available",
			},
		},
		Spec: corev1.PodSpec{
			...
			Volumes: []corev1.Volume{
				{
					...
					VolumeSource: corev1.VolumeSource{
						PersistentVolumeClaim: &corev1.PersistentVolumeClaimVolumeSource{
							ClaimName: "Vulerian-projects-mycmp-app",
						},
					},
				},
				{
					...
					VolumeSource: corev1.VolumeSource{
						PersistentVolumeClaim: &corev1.PersistentVolumeClaimVolumeSource{
							ClaimName: "Vulerian-shared-data-mycmp-app",
						},
					},
				},
			},
...
func Test_createPodFromComponent(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			ctx = Vuleriancontext.WithEffectiveDevfileObj(ctx, &devfileObj)
			ctx = Vuleriancontext.WithApplication(ctx, tt.args.appName)
			ctx = Vuleriancontext.WithComponentName(ctx, tt.args.componentName)
			ctx = Vuleriancontext.WithWorkingDirectory(ctx, "/tmp/dir")
