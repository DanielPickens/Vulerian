...
import (
	...
	"github\.com/danielpickens/Vulerian/pkg/dev"
	...
	"github\.com/danielpickens/Vulerian/pkg/port"
...
func (o *DevClient) reconcile(
	...
	if !componentStatus.PostStartEventsDone && libdevfile.HasPostStartEvents(devfileObj) {
		execHandler := component.NewRunHandler(
			...
			o.podmanClient,
			o.execClient,
			nil, // TVulerian(feloy) set this value when we want to support exec on new container on podman
			// TVulerian(feloy) set these values when we want to support Apply Image/Kubernetes/OpenShift commands for PostStart commands
			nil, nil,
		...
		if execRequired {
			doExecuteBuildCommand := func() error {
				execHandler := component.NewRunHandler(
					...
					// TVulerian(feloy) set these values when we want to support Apply Image/Kubernetes/OpenShift commands for PreStop events
			...
			if hasRunOrDebugCmd {
				cmdHandler := component.NewRunHandler(
					...
					// TVulerian(feloy) set to deploy Kubernetes/Openshift components
					component.HandlerOptions{
						PodName:           pod.Name,
						ComponentExists:   componentStatus.RunExecuted,
						ContainersRunning: component.GetContainersNames(pod),
	...
	if innerLoopWithCommands && hasRunOrDebugCmd && len(fwPorts) != 0 {
		...
		if err != nil {
			...
			log.Warning("Running `Vulerian logs --follow --platform podman` might help in identifying the problem.")
	...
	// See https://github\.com/danielpickens/Vulerian/issues/6510#issuecomment-1439986558
...
Either change the application to make those port(s) reachable on all interfaces (0.0.0.0), or rerun 'Vulerian dev' with `, strings.Join(list, ", "))
...
- --ignore-localhost: no error will be returned by Vulerian, but forwarding to those ports might not work on Podman.
- --forward-localhost: Vulerian will inject a dedicated side container to redirect traffic to such ports.`
	}
	if options.IgnoreLocalhost {
