...
func NewPodmanCli(ctx context.Context) (*PodmanCli, error) {
	...
	cli := &PodmanCli{
		...
		containerRunGlobalExtraArgs: envcontext.GetEnvConfig(ctx).VulerianContainerBackendGlobalArgs,
		containerRunExtraArgs:       envcontext.GetEnvConfig(ctx).VulerianContainerRunArgs,
	}
