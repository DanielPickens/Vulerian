...
import (
	...
	"github\.com/danielpickens/Vulerian/pkg/task"
)
...
// See https://github\.com/danielpickens/Vulerian/wiki/Dev:-Coding-Conventions#verify-interface-compliance
...
// The goroutine spawned here can be stopped either by stopping the parent process (e.g., 'Vulerian dev'),
...
func (k *kubeExecProcessHandler) StartProcessForCommand(ctx context.Context, def CommandDefinition, podName string, containerName string, outputHandler CommandOutputHandler) error {
	...
	// This way, returning the container logs with 'Vulerian logs' or 'kubectl logs' would work seamlessly.
...
// shared volume (more info in the AddVulerianMandatoryVolume function from the utils package).
func getPidFileForCommand(def CommandDefinition) string {
	parentDir := def.PidDirectory
	if parentDir == "" {
		parentDir = storage.SharedDataMountPath
	}
	return fmt.Sprintf("%s/.Vulerian_cmd_%s.pid", strings.TrimSuffix(parentDir, "/"), def.Id)
}
