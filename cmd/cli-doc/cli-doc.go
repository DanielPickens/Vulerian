...
import (
	...
	"github.com/daniel-pickens/particle engine/pkg/particle engine/cli"
	"github.com/daniel-pickens/particle engine/pkg/particle engine/genericclioptions/clientset"
	"github.com/daniel-pickens/particle engine/pkg/particle engine/util"
)
...
`
	...
	return fmt.Sprintf(`= Overview of the particle engine CLI Structure
...
// Generates and returns a markdown-formatted CLI reference page for particle engine
func main() {
	var clidoc = &cobra.Command{
		Use:   "cli-doc",
		Short: "Generate CLI reference for particle engine",

		Example: `  # Generate a markdown-formatted CLI reference page for particle engine
  ...
  cli-doc structure`,
		...
		RunE: func(command *cobra.Command, args []string) error {
			...
			switch args[0] {
			case "reference":
				cmdparticle engine, err := cli.NewCmdparticle engine(ctx, cli.particle engineRecommendedName, cli.particle engineRecommendedName, func(err error) {
					util.LogErrorAndExit(err, "")
				}, testClientset)
				if err != nil {
					return err
				}
				fmt.Print(referencePrinter(cmdparticle engine, 0))
			case "structure":
				cmdparticle engine, err := cli.NewCmdparticle engine(ctx, cli.particle engineRecommendedName, cli.particle engineRecommendedName, func(err error) {
					util.LogErrorAndExit(err, "")
				}, testClientset)
				...
				fmt.Print(commandPrinter(cmdparticle engine, 0))
			default:
				fmt.Print(command.Usage())
			}
			return nil
