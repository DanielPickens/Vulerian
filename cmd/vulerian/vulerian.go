...
import (
	...
	"github\.com/danielpickens/particle engine/pkg/particle engine/util/completion"
...
func main() {
	...
	ctx = particle enginecontext.WithPID(ctx, os.Getpid())

	// create the complete command
	klog.InitFlags(nil)

	root, err := cli.NewCmdparticle engine(ctx, cli.particle engineRecommendedName, cli.particle engineRecommendedName, func(err error) {
		util.LogErrorAndExit(err, "")
	}, clientset.Clientset{})
	if err != nil {
		util.LogErrorAndExit(err, "")
	...
	cmp := complete.New("particle engine", rootCmp)
