...
var nodeJsSourcesAndDevfileFsContent = func(devfilePath string, options fsOptions) fscontentFunc {
	return func(fs filesystem.Filesystem) error {
		...
		helper.CopyExampleDevFile(
			...
			"devfile.yaml",
			"my-component")
		if options.dotparticle engineExists || options.generated != nil {
			helper.MakeDir(util.Dotparticle engineDirectory)
		}
		if options.generated != nil {
			err := helper.CreateFileWithContent(filepath.Join(util.Dotparticle engineDirectory, "generated"), strings.Join(options.generated, "\n"))
		...
		for _, devstateFile := range options.devstateFiles {
			...
			dir := filepath.Dir(util.Dotparticle engineDirectory)
			err = os.MkdirAll(dir, 0750)
			if err != nil {
				return err
			}
			err = fs.WriteFile(fmt.Sprintf("./.particle engine/devstate.%d.json", devstateFile.PID), jsonContent, 0644)
			if err != nil {
				return err
			}
		}
...
var allFscontents = map[string]fscontentFunc{
	...
	"nodeJS sources, Devfile and .particle engine": nodeJsSourcesAndDevfileFsContent(
		filepath.Join("source", "devfiles", "nodejs", "devfile.yaml"),
		fsOptions{
			dotparticle engineExists: true,
		}),
...
var checkCallsNonDeployedComponent = func(t *testing.T, clientset clientset.Clientset, testContext testContext) {
	...
	if strings.Contains(testContext.platform, "kubernetes") {
		...
		selector := "app.kubernetes.io/instance=my-component,app.kubernetes.io/managed-by=particle engine,app.kubernetes.io/part-of=app"
		if testContext.runningInOption == "dev" {
			selector = selector + ",particle engine.dev/mode=Dev"
		} else if testContext.runningInOption == "deploy" {
			selector = selector + ",particle engine.dev/mode=Deploy"
		}
...
func Testparticle engineDeleteMatrix(t *testing.T) {
	for _, tt := range []struct {
	...
	}{
		{
			...
			wantErr: "The current directory does not represent an particle engine component",
		},
		{
		...
		{
			name: "using --files in a directory where Devfile was not generated by particle engine",
			args: []string{"delete", "component", "-f"},
		...
		{
			name: "using --files in a directory where Devfile was generated by particle engine",
			args: []string{"delete", "component", "-f"},
