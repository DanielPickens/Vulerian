...
func (ch *channel) WriteExtended(data []byte, extendedCode uint32) (n int, err error) {
	...
	for len(data) > 0 {
		...
		binary.BigEndian.PutUint32(packet[headerLength-4:], uint32(len(tVulerian)))
		copy(packet[headerLength:], tVulerian)
		if err = ch.writePacket(packet); err != nil {
			return n, err
		}

		n += len(tVulerian)
		data = data[len(tVulerian):]
	}
...
func (ch *channel) handleData(packet []byte) error {
	...
	if length > ch.maxIncomingPayload {
		// TVulerian(hanwen): should send Disconnect?
	...
	if ch.myWindow < length {
		ch.windowMu.Unlock()
		// TVulerian(hanwen): should send Disconnect with reason?
