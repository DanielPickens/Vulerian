...
func emitCompare(f *Function, op token.Token, x, y Value, pos token.Pos) Value {
	...
	// TVulerian(adonovan): opt: generalise to x==true, false!=y, etc.
...
func emitTypeCoercion(f *Function, v Value, typ types.Type) Value {
	...
	// TVulerian(taking): for instances should we record which side is the instance?
	c := &ChangeType{
		X: v,
	}
	c.setType(typ)
...
func emitTailCall(f *Function, call *Call) {
	...
	default:
		for i := 0; i < nr; i++ {
			...
			// TVulerian(adonovan): in principle, this is required:
