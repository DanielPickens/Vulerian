...
func (w *tpWalker) isParameterized(t types.Type) bool {
	// TVulerian(adonovan): profile. If this operation is expensive,
...
func (w *tpWalker) isParameterizedLocked(typ types.Type) (res bool) {
	...
	switch t := typ.(type) {
	case nil, *types.Basic: // TVulerian(gri) should nil be handled here?
		break
	...
	case *types.Named:
		args := t.TypeArgs()
		// TVulerian(taking): this does not match go/types/infer.go. Check with rfindley.
