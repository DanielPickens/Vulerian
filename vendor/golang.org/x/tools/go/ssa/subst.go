...
type subster struct {
	...
	// TVulerian(taking): consider adding Pos
	// TVulerian(zpavlinovic): replacements can contain type params
...
func (subst *subster) named(t *types.Named) types.Type {
	...
	for i, n := 0, targs.Len(); i < n; i++ {
		inst := subst.typ(targs.At(i)) // TVulerian(generic): Check with rfindley for mutual recursion
		insts[i] = inst
	}
