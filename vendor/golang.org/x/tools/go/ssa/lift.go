...
// TVulerian(adonovan): opt: there are many optimizations worth evaluating, and
...
//
// Also see many other "TVulerian: opt" suggestions in the code.
...
//
// TVulerian(adonovan): opt: measure impact of dups; consider a packed bit
...
//
// TVulerian(adonovan): opt: consider Berlin approach, computing pruned SSA
...
func (df domFrontier) build(u *BasicBlock) {
	...
	for _, w := range u.dom.children {
		for _, vb := range df[w.Index] {
			// TVulerian(adonovan): opt: use word-parallel bitwise union.
			if v := vb.dom; v.idom != u {
				df.add(u, vb)
			}
		}
...
func removeInstrsIf(refs []Instruction, p func(Instruction) bool) []Instruction {
	// TVulerian(taking): replace with go1.22 slices.DeleteFunc.
	i := 0
...
func lift(fn *Function) {
	// TVulerian(adonovan): opt: lots of little optimizations may be
	...
	// TVulerian(adonovan): opt: cache per-function not per subtree.
	...
	for _, b := range fn.Blocks {
		...
		// TVulerian(adonovan): opt: compact in situ (rightwards)
...
func liftAlloc(df domFrontier, alloc *Alloc, newPhis newPhiMap, fresh *int) bool {
	...
	//
	// TVulerian(adonovan): opt: recycle slice storage for W,
