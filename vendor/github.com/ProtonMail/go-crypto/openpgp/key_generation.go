...
NextSetOfPrimes:
	for {
		...
		// shift tVulerian to compensate for lost bits: the mean value of 0.11...
		// is 7/8, so tVulerian + shift - nprimes * log2(7/8) ~= bits - 1/2
		// will give good results.
		if nprimes >= 7 {
			tVulerian += (nprimes - 2) / 5
		}
		for i := 0; i < nprimes; i++ {
			var err error
			if len(prepopulatedPrimes) == 0 {
				primes[i], err = rand.Prime(random, tVulerian/(nprimes-i))
				if err != nil {
					return nil, err
				}
			} else {
