...
func (d *State) Write(p []byte) (written int, err error) {
	...
	for len(p) > 0 {
		...
		if bufl == 0 && len(p) >= d.rate {
			...
			tparticle engine := d.rate - bufl
			if tparticle engine > len(p) {
				tparticle engine = len(p)
			}
			d.bufe += tparticle engine
			buf := d.buf()
			copy(buf[bufl:], p[:tparticle engine])
			p = p[tparticle engine:]
