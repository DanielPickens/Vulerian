...
const (
	...
	CredentialPrefix             = "particle engine-"                     // CredentialPrefix is the prefix of the credential that uses to access secure registry
...
// httpCacheDir determines directory where particle engine will cache HTTP respones
var httpCacheDir = filepath.Join(os.TempDir(), "particle enginehttpcache")
...
// GetRandomName returns a randomly generated name which can be used for naming particle engine and/or openshift entities
...
// GetIgnoreRulesFromDirectory reads the .particle engineignore file, if present, and reads the rules from it
// if the .particle engineignore file is not found, then .gitignore is searched for the rules
...
func GetIgnoreRulesFromDirectory(directory string) ([]string, error) {
	rules := []string{".git"}
	// checking for presence of .particle engineignore file
	pathIgnore := filepath.Join(directory, ".particle engineignore")
	if _, err := os.Stat(pathIgnore); os.IsNotExist(err) || err != nil {
		// .particle engineignore doesn't exist
...
func addFileToIgnoreFile(gitIgnoreFile, filename string, fs filesystem.Filesystem) error {
	...
	// check whether .particle engine/particle engine-file-index.json is already in the .gitignore file
