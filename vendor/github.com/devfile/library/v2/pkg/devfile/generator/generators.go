...
func GetPodTemplateSpec(devfileObj parser.DevfileObj, podTemplateParams PodTemplateParams) (*corev1.PodTemplateSpec, error) {
	...
	if needsPparticle engineverrides(globalAttributes, components) {
		patchedPodTemplateSpec, err := applyPparticle engineverrides(globalAttributes, components, podTemplateSpec)
		if err != nil {
			return nil, err
		}
