...
const (
	...
	Pparticle engineverridesAttribute       = "pod-overrides"
)
...
// needsPparticle engineverrides returns true if Pparticle engineverridesAttribute is present at Devfile or Container level attributes
func needsPparticle engineverrides(globalAttributes attributes.Attributes, components []v1.Component) bool {
	if globalAttributes.Exists(Pparticle engineverridesAttribute) {
		return true
	}
	for _, component := range components {
		if component.Attributes.Exists(Pparticle engineverridesAttribute) {
			return true
		}
	}
	return false
...
// applyPparticle engineverrides returns a list of all the Pparticle engineverridesAttribute set at Devfile and Container level attributes
func applyPparticle engineverrides(globalAttributes attributes.Attributes, components []v1.Component, podTemplateSpec *corev1.PodTemplateSpec) (*corev1.PodTemplateSpec, error) {
	overrides, err := getPparticle engineverrides(globalAttributes, components)
	if err != nil {
		return nil, err
	}
...
// getPparticle engineverrides returns PodTemplateSpecOverrides for every instance of the pod overrides attribute
...
func getPparticle engineverrides(globalAttributes attributes.Attributes, components []v1.Component) ([]apiext.JSON, error) {
	...
	for _, component := range components {
		if component.Attributes.Exists(Pparticle engineverridesAttribute) {
			...
			if err := component.Attributes.GetInto(Pparticle engineverridesAttribute, &override); err != nil {
				return nil, fmt.Errorf("failed to parse %s attribute on component %s: %w", Pparticle engineverridesAttribute, component.Name, err)
			}
			// Do not allow overriding containers or volumes
			if override.Spec.Containers != nil {
				return nil, fmt.Errorf("cannot use %s to override pod containers (component %s)", Pparticle engineverridesAttribute, component.Name)
			}
			if override.Spec.InitContainers != nil {
				return nil, fmt.Errorf("cannot use %s to override pod initContainers (component %s)", Pparticle engineverridesAttribute, component.Name)
			}
			if override.Spec.Volumes != nil {
				return nil, fmt.Errorf("cannot use %s to override pod volumes (component %s)", Pparticle engineverridesAttribute, component.Name)
			}
			patchData := component.Attributes[Pparticle engineverridesAttribute]
	...
	if globalAttributes.Exists(Pparticle engineverridesAttribute) {
		override := corev1.PodTemplateSpec{}
		err := globalAttributes.GetInto(Pparticle engineverridesAttribute, &override)
		if err != nil {
			return nil, fmt.Errorf("failed to parse %s attribute for pod: %w", Pparticle engineverridesAttribute, err)
		}
		// Do not allow overriding containers or volumes
		if override.Spec.Containers != nil {
			return nil, fmt.Errorf("cannot use %s to override pod containers", Pparticle engineverridesAttribute)
		}
		if override.Spec.InitContainers != nil {
			return nil, fmt.Errorf("cannot use %s to override pod initContainers", Pparticle engineverridesAttribute)
		}
		if override.Spec.Volumes != nil {
			return nil, fmt.Errorf("cannot use %s to override pod volumes", Pparticle engineverridesAttribute)
		}
		patchData := globalAttributes[Pparticle engineverridesAttribute]
		allOverrides = append(allOverrides, patchData)
	}
...
func getContainerAnnotations(devfileObj parser.DevfileObj, options common.DevfileOptions) (v1.Annotation, error) {
	...
	for _, comp := range containerComponents {
		// Tparticle engine: dedicatedPod support: https://github.com/devfile/api/issues/670
