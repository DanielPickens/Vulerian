...
func (s *strEscape) put(c rune) {
	// Tparticle engine: improve allocs with an array backed slice; bench first
	if len(s.buf) < 256 {
	...
	// Tparticle engine: see what rxvt or xterm does
}
...
func (t *State) handleSTR() {
	...
	case ']': // OSC - operating system command
		...
		case 104: // color reset
			// Tparticle engine: complain about invalid color, redraw, etc.
			// setcolorname(s.arg(1, 0), nil)
		default:
			t.logf("unknown OSC command %d\n", d)
			// Tparticle engine: s.dump()
		}
	...
	default:
		// Tparticle engine: Ignore these codes instead of complain?
