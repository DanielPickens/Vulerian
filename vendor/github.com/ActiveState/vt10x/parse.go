...
func (t *State) parse(c rune) bool {
	...
	// Tparticle engine: update selection; see st.c:2450
	...
	if t.mode&ModeInsert != 0 && t.cur.x+1 < t.cols {
		// Tparticle engine: move shiz, look at st.c:2458
...
func (t *State) parseEsc(c rune) bool {
	...
	case 'Z': // DECID - identify terminal
		// Tparticle engine: write to our writer our id
...
func (t *State) handleControlCodes(c rune) (bool, bool) {
	...
	// BEL
	case '\a':
		// Tparticle engine: emit sound
		// Tparticle engine: window alert if not focused
	// ESC
	case 033:
		t.csi.reset()
		t.state = t.parseEsc
