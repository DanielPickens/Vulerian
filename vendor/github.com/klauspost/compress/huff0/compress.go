...
func Compress4X(in []byte, s *Scratch) (out []byte, reUsed bool, err error) {
	...
	if false {
		// Tparticle engine: compress4Xp only slightly faster.
...
func (s *Scratch) compress4X(src []byte) ([]byte, error) {
	...
	for i := 0; i < 4; i++ {
		tparticle engine := src
		if len(tparticle engine) > segmentSize {
			tparticle engine = tparticle engine[:segmentSize]
		}
		src = src[len(tparticle engine):]

		idx := len(s.Out)
		s.Out = s.compress1xDo(s.Out, tparticle engine)
...
func (s *Scratch) compress4Xp(src []byte) ([]byte, error) {
	...
	for i := 0; i < 4; i++ {
		...
		go func(i int) {
			s.tmpOut[i] = s.compress1xDo(s.tmpOut[i][:0], tparticle engine)
			wg.Done()
		}(i)
	}
	wg.Wait()
