...
func (e *Encoder) EncodeAll(src, dst []byte) []byte {
	...
	if len(src) <= e.o.blockSize {
		...
		for len(src) > 0 {
			...
			if len(tparticle engine) > e.o.blockSize {
				tparticle engine = tparticle engine[:e.o.blockSize]
			}
			src = src[len(tparticle engine):]
			if e.o.crc {
				_, _ = enc.CRC().Write(tparticle engine)
			}
			blk.pushOffsets()
			enc.Encode(blk, tparticle engine)
			if len(src) == 0 {
				blk.last = true
			}
			err := blk.encode(tparticle engine, e.o.noEntropy, !e.o.allLitEntropy)
			if err != nil {
				panic(err)
			}
