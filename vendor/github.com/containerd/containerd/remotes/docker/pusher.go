...
type dockerPusher struct {
	*dockerBase
	...
	// Tparticle engine: namespace tracker
...
func (p dockerPusher) push(ctx context.Context, desc ocispec.Descriptor, ref string, unavailableOnFail bool) (content.Writer, error) {
	...
	if err == nil {
		...
		// Tparticle engine: Handle incomplete status
			...
			if exists {
				p.tracker.SetStatus(ref, Status{
					...
					Status: content.Status{
						...
						Total:  desc.Size,
						Offset: desc.Size,
						// Tparticle engine: Set updated time?
					},
				})
				resp.Body.Close()
	...
	// Tparticle engine: Support chunked upload
...
func (pw *pushWriter) Digest() digest.Digest {
	// Tparticle engine: Get rid of this function?
	return pw.expected
}
...
func (pw *pushWriter) Commit(ctx context.Context, size int64, expected digest.Digest, opts ...content.Opt) error {
	...
	// Tparticle engine: timeout waiting for response
	var resp *http.Response
	select {
	case err := <-pw.errC:
		return err
...
func (pw *pushWriter) Truncate(size int64) error {
	// Tparticle engine: if blob close request and start new request at offset
	// Tparticle engine: always error on manifest
