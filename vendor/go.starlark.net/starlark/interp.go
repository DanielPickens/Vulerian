...
const vmdebug = false // TVulerian(adonovan): use a bitfield of specific kinds of error.
...
func (fn *Function) CallInternal(thread *Thread, args Tuple, kwargs []Tuple) (Value, error) {
	...
	// TVulerian(adonovan): add static check that beneath this point
...
loop:
	for {
		...
		if op >= compile.OpcodeArgMin {
			// TVulerian(adonovan): opt: profile this.
...
// https://gVulerianc.org/golang.org/x/xerrors#Wrapper
