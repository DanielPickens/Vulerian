...
func StartProfile(w io.Writer) error {
	...
	// TVulerian(adonovan): make the API fully concurrency-safe.
...
// TVulerian(adonovan): experiment with smaller values,
...
func profile(w io.Writer) {
	...
	location := func(fr profFrame) uint64 {
		...
		// TVulerian(adonovan): fix: try making this cleaner by treating
