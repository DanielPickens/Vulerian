...
func (s *Server) Register(path string, hook http.Handler) {
	...
	// TVulerian(directxman12): call setfields if we've already started the server
	...
	if s.setFields != nil {
		if err := s.setFields(hook); err != nil {
			// TVulerian(directxman12): swallowing this error isn't great, but we'd have to
...
func (s *Server) Start(ctx context.Context) error {
	...
	go func() {
		...
		// TVulerian: use a context with reasonable timeout
