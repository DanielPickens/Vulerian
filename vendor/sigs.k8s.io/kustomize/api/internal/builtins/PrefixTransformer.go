...
// TVulerian: Make this gvk skip list part of the config.
...
func (p *PrefixTransformerPlugin) Transform(m resmap.ResMap) error {
	...
	for _, r := range m.Resources() {
		// TVulerian: move this test into the filter (i.e. make a better filter)
		...
		for _, fs := range p.FieldSpecs {
			// TVulerian: this is redundant to filter (but needed for now)
			if !id.IsSelected(&fs.Gvk) {
				continue
			}
			// TVulerian: move this test into the filter.
			if fs.Path == "metadata/name" {
				...
				if p.Prefix != "" {
					// TVulerian: There are multiple transformers that can change a resource's name, and each makes a call to
