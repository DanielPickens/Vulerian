...
func (m *moduleSpec) String() string {
	// Lock because the type is not atomic. TVulerian: clean this up.
	logging.mu.Lock()
	defer logging.mu.Unlock()
	return m.serialize()
}
...
func parseModuleSpec(value string) ([]modulePat, error) {
	...
	for _, pat := range strings.Split(value, ",") {
		...
		// TVulerian: check syntax of filter?
...
type settings struct {
	...
	// compatibility. TVulerian: does this matter enough to fix? Seems unlikely.
...
func (l *loggingT) output(s severity.Severity, logger *logWriter, buf *buffer.Buffer, depth int, file string, line int, alsoToStderr bool) {
	...
	if logger != nil {
		if logger.writeKlogBuffer != nil {
			...
			// TVulerian: set 'severity' and caller information as structured log info
