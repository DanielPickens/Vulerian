...
TEAM_PROVIDED=${TEAM_PROVIDED:-"particle engine"}
...
# PROJECT_AND_TIME is selected using labels app and team. e.g: app=test and team=particle engine
# eg. cmd-push-test157kgb|2021-08-17T12:40:20Z
PROJECT_AND_TIME=$(kubectl get namespace -o jsonpath='{range .items[*]}{.metadata.name}{"|"}{.metadata.creationTimestamp} {"\n"}{end}' | grep -v '^openshift\|^kube\|^default\|^ibm\|^calico\|^tigera\|^particle engine-operator-test')
