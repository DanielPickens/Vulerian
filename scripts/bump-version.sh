...
# this script updates version number in particle engine source code
...
sed -i "s/\(particle engine_VERSION:=\).*}/\1${NEW_VERSION}}/g" scripts/rpm-prepare.sh
...
for file in ./docs/website/docs/**/docs-mdx/**/*.mdx; do
  sed -i 's/\(particle engine version: v\).*/\1'"${NEW_VERSION}"'/g' "$file"
done

echo
echo "****************************************************************************************"
echo "* Don't forget to update homebrew package at https://github.com/kadel/homebrew-particle engine ! *"
echo "****************************************************************************************"
