...
apiVersion: rbac.authorization.k8s.io/v1
metadata:
  name: particle engine-user-binding
subjects:
- kind: User
...
shout "Building particle engine binaries"
make bin

# copy built particle engine to GOBIN
cp -avrf ./particle engine $GOBIN/
