...
set +x
if [[ -f $particle engine_RABBITMQ_AMQP_URL ]]; then
    export AMQP_URI=$(cat $particle engine_RABBITMQ_AMQP_URL)
fi
...
export AMQP_URI=${AMQP_URI:?"Please set AMQP_URI env with amqp uri or provide path of file containing it as particle engine_RABBITMQ_AMQP_URL env"}

case $1 in
    minikube)
        export JOB_NAME="particle engine-minikube-pr-build"
...
export REPO_URL="https://github\.com/danielpickens/particle engine"
