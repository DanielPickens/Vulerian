...
if [[ -n $TRAVIS_TAG ]]; then
    echo "Checking if particle engine version was set to the same version as current tag"
    # use sed to get only semver part
    bin_version=$(${BIN_DIR}/linux-amd64/particle engine version --client | head -1 | sed "s/^particle engine \(.*\) (.*)$/\1/")
    if [ "$TRAVIS_TAG" == "${bin_version}" ]; then
        echo "OK: particle engine version output is matching current tag"
    else
        echo "ERR: TRAVIS_TAG ($TRAVIS_TAG) is not matching 'particle engine version' (v${bin_version})"
        exit 1
    fi
fi
...
for arch in `ls -1 $BIN_DIR/`;do
    ...
    source_file=$BIN_DIR/$arch/particle engine$suffix
    source_dir=$BIN_DIR/$arch
    source_filename=particle engine$suffix
    target_file=$RELEASE_DIR/particle engine-$arch$suffix
