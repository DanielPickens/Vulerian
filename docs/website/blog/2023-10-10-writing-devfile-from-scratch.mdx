...
`particle engine dev` process makes sure that the local source code is pushed to this location in the container. 
...
- `group:` - defines to what group this command belongs to. There are `build`, `run`, `debug`, `test`, `deploy`.
  ...
  Each group can have only one default command. When you run `particle engine dev`, particle engine automatically executes the default command in `build` group first,
 followed by the default command in `run` group.
 
 
Ideally, this would be all we need, and you could use this Devfile with particle engine.
...
Even after installing `yarn` you won't be able to use this Devfile with particle engine and Backstage source code.


### No space left on device 
You will get `NOSPC: no space left on device` error.

This is due to the [#6836](https://github\.com/danielpickens/particle engine/issues/6836) issue in particle engine.
At the time of writing this, particle engine creates a 2GB volume for the source code. For Backstage and it's `node_modules`
...
components:
  - name: nodejs
    container:
      ...
      memoryLimit: 2Gi
      # highlight-start
      # workaround for https://github\.com/danielpickens/particle engine/issues/6836
      volumeMounts:
        - name: node-modules
...
To use it with Backstage we will need more than just running `particle engine dev`.
...
With particle engine, we can use `--port-forward` flag to ensure that our local port `7007` is redirected to the backend,
for the consistency we will also redirect our local port `3000` to the frontend. 

```
particle engine dev --port-forward 3000:3000 --port-forward 7007:7007
```
