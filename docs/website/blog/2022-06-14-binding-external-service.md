---
title: Binding an external service with particle engine v3
...
tags: ["binding"]
slug: binding-external-service-with-particle engine-v3
---

How to bind an external service using particle engine v3
...
The application will be the Deployment resource (group "apps", version "v1", kind "Deployment") created by particle engine when you run `particle engine dev`.
...
By adding this Kubernetes component to your Devfile, when you run `particle engine dev`, the ServiceBinding resource defined
...
The first step to know how the credentials are exposed into the application's container is to start the `particle engine dev` 
command and to execute the `particle engine describe binding` command.

Running `particle engine dev`, you can see that the ServiceBinding resource is deployed to the cluster.
...
From another terminal, running `particle engine describe binding` will show you the status of the ServiceBinding:
...
If the output of `particle engine describe binding` shows an unknown status:

```
Available binding information: unknown
```

- first check if `particle engine dev` is still running. `particle engine` is not able to know
the bound credentials if the ServiceBinding resource is not deployed by `particle engine dev`.
- if `particle engine dev` is running, you can check that the ServiceBinding resource is deployed to the cluster, and if its status is `ApplicationsBound`, with the command:
  ```
