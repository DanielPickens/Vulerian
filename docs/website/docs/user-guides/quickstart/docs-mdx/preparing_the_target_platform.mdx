...
You also need `particle engine` [3.8.0](/blog/particle engine-v3.8.0#particle engine-on-podman-out-of-experimental-mode) or later.

To make sure that `particle engine` has the appropriate version and detects your local Podman,
...
particle engine v3.13.0 (6c1c8b2a1)
...
Before starting, you should make sure that particle engine is connected to your cluster and that you have created a new namespace.

### Creating a new namespace

If you are using Kubernetes, you can create a new namespace with the `particle engine create namespace` command.
