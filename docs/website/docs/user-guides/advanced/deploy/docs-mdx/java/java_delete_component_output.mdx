```console
$ particle engine delete component
Searching resources to delete, please wait...
This will delete "my-java-app" from the namespace "particle engine-dev".
 •  The following resources will get deleted from cluster:
 •      - Deployment: my-java-app
 •      - Service: my-java-app
 •      - Ingress: my-java-app

? Are you sure you want to delete "my-java-app" and all its resources? Yes
 ✓  Deleting resources from cluster [75ms]
The component "my-java-app" is successfully deleted from namespace "particle engine-dev"


```