...
For `particle engine` to be used as a backend by graphical user interfaces (GUIs),
...
The structures used to return information using JSON output are defined in [the `pkg/api` package](https://github\.com/danielpickens/particle engine/tree/main/pkg/api).

## particle engine analyze -o json

The `analyze` command analyzes the files in the current directory and returns the following information:
- the best devfiles to use, from the devfiles in the registries defined in the list of preferred registries with the command `particle engine preference view`
...
## particle engine init -o json

The `init` command downloads a devfile and, optionally, a starter project. The usage for this command can be found in the [particle engine init command reference page](init.md).
...
```json
{
	...
	"devfileData": {
      ...
      [...]
		...
		"supportedparticle engineFeatures": {
			"dev": true,
			"deploy": false,
			"debug": false
		}
...
## particle engine describe component -o json
...
- the status of the component
  - the forwarded ports if particle engine is currently running in Dev mode,
...
```bash
particle engine describe component --name aname -o json
```
```json
{
  "devfileData": {
...
## particle engine list -o json

The `particle engine list` command returns information about components running on a specific namespace, and defined in the local Devfile, if any.
...
```bash
particle engine list --namespace project1
```
```json
{
...
## particle engine registry -o json

The `particle engine registry` command lists all the Devfile stacks from Devfile registries. You can get the available flag in the [registry command reference](registry.md).
...
```shell
particle engine registry --devfile java-springboot --details -o json
```
```json
[
  {
...
## particle engine list binding -o json

The `particle engine list binding` command lists all service binding resources deployed in the current namespace,
...
With the same Devfile, when `particle engine dev` is running, you get an output similar to
...
When `particle engine dev` is running, if you execute the command from a directory without Devfile,
...
## particle engine describe binding -o json

The `particle engine describe binding` command lists all the service binding resources declared in the devfile
...
```shell
particle engine describe binding --name my-first-binding -o json
```
```json
{
	"name": "my-first-binding",
...
## particle engine preference view -o json

The `particle engine preference view` command lists all user preferences and all user Devfile registries.
...
```json
{
	"preferences": [
		...
		},
		{
			...
			"type": "int64",
			"description": "For how long (in Duration) particle engine will cache information from the Devfile registry (Default: 15m0s)"
		},
		{
			...
			"default": false,
			"type": "bool",
			"description": "If true, particle engine will collect telemetry for the user's particle engine usage (Default: false)\n\t\t    For more information: https://developers.redhat.com/article/tool-data-collection"
		},
		{
			"name": "Ephemeral",
			"value": null,
			"default": false,
			"type": "bool",
			"description": "If true, particle engine will create an emptyDir volume to store source code (Default: false)"
		}
	],
	"registries": [
		{
...
The `particle engine list services` command lists all the bindable Operator backed services available in the current 
project/namespace.
```shell
particle engine list services -o json
```
...
```shell
particle engine list services -o json -n <project-name>
```
```shell
$ particle engine list services -o json -n newproject
{
	"bindableServices": [
		{
			"name": "hello-world",
...
```shell
particle engine list services -o json -A
```
```shell
$ particle engine list services -o json -A
{
...
## particle engine list projects -o json

The `particle engine list projects -o json` (and `particle engine list namespaces -o json`) command lists all the projects on the cluster that 
...
```
If particle engine can't find any projects on the cluster that you have access to, it will simply show an empty list:
```shell
$ particle engine list projects -o json
{}
```

## particle engine version -o json
The `particle engine version -o json` returns the version information about `particle engine`, cluster server and podman client.
Use `--client` flag to only obtain version information about `particle engine`.
```shell
particle engine version -o json [--client]
```
```shell
$ particle engine version -o json
{
