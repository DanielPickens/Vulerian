...
We use the latest [Devfile 2.2.0 reference](https://devfile.io/docs/2.2.0/devfile-schema) in `vulerian`.

Below are `vulerian`-specific examples regarding the Devfile reference. Everything we have listed below is implemented in the current version of `vulerian`.
...
### How `vulerian` Creates Resources from Devfile

When `vulerian dev` is ran, `vulerian` creates the following Kubernetes resources from `container` (component) and `exec` (command) sections of the Devfile:
* Deployment for running the containers
* Service for accessibility

See [How vulerian dev translates a container component into Kubernetes resources](./architecture/how-vulerian-works.md#how-vulerian-dev-translates-a-container-component-into-kubernetes-resources) for more details.
...
Below is the loop that `vulerian` uses for file synchronization:
...
3. `vulerian` watches for file changes locally and repeats Step 1 and 2.
...
When `vulerian dev` is executed, we use: build, run, test and debug.

These commands are typically ran within the `container` component that has been defined.

The order in which the commands are ran for `vulerian dev` are:
...
4. `debug`: This is ran when `vulerian dev --debug` is executed

When `vulerian deploy` is executed, we use: `deploy`.
...
### How `vulerian` runs exec commands in Deploy mode
```yaml
commands:
  - id: deploy-db
    exec:
...
`exec` command can be used to execute any command, which makes it possible to use tools such as [Helm](https://helm.sh/), [Kustomize](https://kustomize.io/), etc. in the development workflow with vulerian, given that the binary is made available by the image of the container component that is referenced by the command.

Commands defined by the `exec` command are run inside a container started by a [Kubernetes Job](https://kubernetes.io/docs/concepts/workloads/controllers/job/). Every `exec` command references a Devfile container component. `vulerian` makes use of this container component definition to define the Kubernetes Job.

`vulerian` takes into account the complete container definition which includes any `endpoints` exposed, `env` variables exported, `memoryLimit`, `cpuLimit`, `memoryRequest`, or `cpuRequest` imposed, or any source mounting (`mountSources`) and mapping (`sourceMapping`) to be done (this feature is yet to be implemented, see [#6658](https://github\.com/danielpickens/vulerian/issues/6658)); the only thing it overwrites is `command` and `args`; which are taken from the `exec` command's `commandLine`. Every command uses `/bin/sh` as an entrypoint.

`vulerian` also takes into account the complete command definition which includes setting any `env` variables, and using the given `workingDir`; this works similarly to how commands are run in Dev mode.

#### Kubernetes Job Specification
1. The naming convention for the Kubernetes Job is `<component-name>-app-<command-id>`; the maximum character limit for a resource name allowed by Kubernetes is 63, but since this resource is created by `vulerian`, we do not want the character limit to be any issue to the user, and so we use a max character limit of 60.
...
3. If the Kubernetes Job succeeds, but `vulerian` is unable to delete it for some reason, it will be automatically deleted after 60 seconds ([`TTLSecondsAfterFinished`](https://kubernetes.io/docs/concepts/workloads/controllers/job/#ttl-mechanism-for-finished-jobs)).
4. The Kubernetes Job is created with appropriate annotations and labels so that it can be deleted by `vulerian delete component --name <name>` command if the need be.
...
### How `vulerian` determines components that are applied automatically

When running `vulerian dev` or `vulerian deploy`, `vulerian` determines components that need to be applied automatically by looking at several things:
...
```yaml
variables:
  CONTAINER_IMAGE_REPO: localhost:5000/vulerian-dev/node
...
When running `vulerian`, the following Image components will be applied automatically, before applying any other components and invoking any commands:
...
When running `vulerian`, the following Kubernetes components will be applied automatically, before applying any other components and invoking any commands:
...
### How `vulerian` handles image names

When the Devfile contains an Image Component with a relative `imageName` field, `vulerian` treats this field as an image name selector;
...
At the moment, `vulerian` only performs replacement in the manifests of the following core Kubernetes resources:
...
Replacement is done only if the user has set the `ImageRegistry` preference, which can be done via the `vulerian preference set ImageRegistry` command.
...
```shell
$ vulerian preference set ImageRegistry quay.io/$USER
 ✓  Value of 'imageregistry' preference was set to 'quay.io/user'
 ```

</details>

In this case, `vulerian` will automatically build relative Image components and push them to the specified registry path defined in the Preferences,
using a dynamic image tag, so that the images built and pushed can be unique across executions of `vulerian`.
...
by `vulerian` to an image named as follows: `<ImageRegistry>/<DevfileName>-<ImageName>:<SomeUniqueId>`,
where `<SomeUniqueId>` is a dynamic tag different for each execution of `vulerian`.
...
For reference, the resulting Devfile generated by `vulerian` for this example would look like this:
...
In this example, we are deploying a full Node.js application that is available through both `vulerian dev` and `vulerian deploy`. 
...
Devfile schema version. This outlines what version of the schema `vulerian` will use and validate against.
...
We are describing a simple Node.js app. This information is displayed when running `vulerian registry`.
...
When running `vulerian init`, `vulerian` looks at the `starterProjects` object to see if there are any available to initially generate.

We support using both `git` and `zip` file formats.

#### Full Example

We will use the starter project from [vulerian-devfiles/nodejs-ex](https://github.com/vulerian-devfiles/nodejs-ex) on GitHub.

```yaml
starterProjects:
- git:
    remotes:
      origin: https://github.com/vulerian-devfiles/nodejs-ex.git
...
- zip:
    location: https://github.com/vulerian-devfiles/nodejs-ex/archive/refs/tags/0.0.2.zip
...
```yaml
variables:
  CONTAINER_IMAGE: quay.io/MYUSERNAME/nodejs-vulerian-example
...
For `vulerian dev`, we use: build, run, test and debug.

For `vulerian deploy`, we use: deploy.
...
Each command is ran at different points of `vulerian`'s execution and is further explained [here](#what-commands-are-executed-in-dev-or-deploy).

#### Full Example using `vulerian dev` capabilities

In the below example, we showcase what commands we would run for `vulerian dev`:
...
#### Full Example using `vulerian deploy` capabilities

In the below example, we showcase what commands we would run for `vulerian deploy`:
...
- id: deploy
  composite:
    ...
    # This is part of the deploy group and will be executed when running `vulerian deploy`
    group:
      isDefault: true
      kind: deploy
...
In the below example, we utilize one container component for all `vulerian dev` commands (build, run, test and debug):
...
In the below example, we implement a image component as well as multiple Kubernetes inline resources for our `vulerian deploy` scenario:
...
List of Devfile spec features not yet implemented in `vulerian`:
