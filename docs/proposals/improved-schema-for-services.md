...
- `particle engine catalog describe service` should include the metadata fields with description so the users can provide then when doing `particle engine service create`. 
- `particle engine catalog list service` wouldn't change at all.
- `particle engine service create` should take flags and dynamically fill the CRD structs - metadata would be used for validation
- `particle engine service create --from-file` would be used by adapter team or whoever wants to provide the whole CRD themselves as a file. this feature is already present and working.
...
The actually order in which these features would be implemented in particle engine would as follows -
...
- when the users executes `particle engine catalog list services`, if there is no cache then we would fetch the `swagger.json`, particle engine wouldn't exit until download. We will show the user "Downloading service information from the cluster ........" while the swagger is downloaded. Also if we the feedback after releasing that the experience is not great then we will consider a non-blocking approach where the user doesn't have to wait the swagger to be downloaded. We aren't doing that for the beginning because non-blocking approach has some more complexities which might not be needed.
- we would store the `swagger.json` in the `~/.particle engine` as `<cluster-url>-swagger.json` to avoid conflicts.
- We would also create a `operator-listing-cache.json` in the `~/.particle engine` which would hold a key value mapping of `cluser-url` => `hash of the latest "particle engine catalog list services"`.
- now when the user runs `particle engine catalog list services` we would check if there is a cache present, if there is then it would compare the hash of the current `particle engine catalog list services` for the cluster with the one present in `operator-listing-cache.json`. If its different we redownload the swagger.json and update the hash of `particle engine catalog list services` in `operator-listing-cache.json`.
...
Sample - `particle engine service create servicebinding.coreos.io/Servicebinding/<version> -p  "services[0].envVarPrefix"="SVC" -p "services[0].namespace"="openshift"`
...
- particle engine also needs to have smart auto-complete which auto selects the version if the CR only has one version.
...
`particle engine catalog service describe strimzi-cluster-operator.v0.21.1/Kafka -o json` would show a flat converted version of `openAPIv3schema` with same structure as `ClusterServiceVersion` with an extra addition of `type` information being present. 
...
`particle engine catalog service describe strimzi-cluster-operator.v0.21.1/Kafka -o json` would show the `ClusterServiceVersion`'s `CRDDescription` shown in the `Fetch ClusterServiceVersion to generate the information` section
...
For any scenerio the user would provide the parameters in flat format i.e. `particle engine service create -p zookeeper.storage.type=ephemeral`. We would build the full parameters map using these flat key values then in case of `openapiv3schema`, validate it against the jsonschema. For `ClusterServiceVersion` we already have the flat path values so we validate them before building the parameters map.
